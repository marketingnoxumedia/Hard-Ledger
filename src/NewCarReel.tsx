...
// ---------------------------------------------------------------------------
// "The new-car tax" reel (NewCarReel) — new-car depreciation, told as the A/B twin of the existing
// DepreciationReel ("The depreciation curve"), which opens and closes declaratively. Here the hook
// and close are rhetorical questions. The most expensive year of a new car is the first: a $40K car
// is worth about $32K within twelve months, and nobody drove it into a wall. A new car sheds ~20% of
// its value in year one — faster than fuel, insurance or repairs — so driving it off the lot quietly
// spends ~$8K in year-one depreciation. The drop is steepest first: ~20% down after one year, ~60%
// after five, worth less than half its sticker before the warranty ends, most of the value leaving
// in the years you can't see it going. Which is why a two- or three-year-old car is a different deal:
// it has already taken the worst of the fall while keeping most of its life — the first owner paid
// for the new smell. New isn't a cost, it's a luxury; the depreciation curve is the real price tag.
// Question hook ("...the most expensive year of owning a new car?") and a rhetorical question close
// ("who really paid for that new smell?"); a ~1.2s post-hook pause (the hook holds through the
// silence before the reveal — house convention); three count-up `stat` beats ($8K year-one drop, then
// 20% and 60% "down" with animated bars for the curve), one red "the curve is the real price tag"
// `impact` card as the mid-tail thesis, and the question close on footage. **Media-dense:** 22 of 25
// beats carry footage, with only 2 flat-black beats ("Value you never see leave.", "Most vanishes
// before year three.") plus the one red card. Durations on exact ElevenLabs spoken-word timestamps at
// +6% pace; music normalized and mixed at 0.17. Total 1744 frames = ~58.1s. Entirely its own fresh
// footage under public/newcar/ — no asset shared with DepreciationReel or any other reel.
// **Brand-safety:** the reel is about cars in general, not any one make — footage varies makes/colors
// and avoids badge-dominated shots, dealership brand signs and readable plates, so no single brand is
// tied to "losing value". Confirm no readable brand badges before publishing. **Illustration, not
// verified data:** the $40K→$32K, ~20%/~60% depreciation curve and ~$8K year-one figure are round,
// illustrative numbers showing the shape of car depreciation — real depreciation varies widely by
...
const SCENES: SceneDef[] = [
  ...
  {dur: 96, kind: 'stat', stat: {pre: 'YOU JUST SPENT', prefix: '$', value: 8, suffix: 'K', post: 'IN YEAR-ONE DEPRECIATION'}, media: {src: 'newcar/p_cash.jpg', type: 'img', effect: 'in'}},
  {dur: 79, kind: 'text', text: 'More than years|of fuel.', highlights: ['more'], size: 88, media: {src: 'newcar/p_pump.jpg', type: 'img', effect: 'in'}},
  {dur: 82, kind: 'text', text: 'The steepest drop|comes first.', highlights: ['steepest'], size: 84, media: {src: 'newcar/p_downhill.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'stat', stat: {pre: 'AFTER ONE YEAR', value: 20, suffix: '%', post: 'DOWN', bar: 20}, media: {src: 'newcar/p_chart.jpg', type: 'img', effect: 'in'}},
  {dur: 46, kind: 'stat', stat: {pre: 'AFTER FIVE YEARS', value: 60, suffix: '%', post: 'DOWN', bar: 60}, media: {src: 'newcar/p_chart2.jpg', type: 'img', effect: 'in'}},
  {dur: 38, kind: 'text', text: 'Less than half|its sticker.', highlights: ['half'], size: 88, media: {src: 'newcar/p_sticker.jpg', type: 'img', effect: 'in'}},
  {dur: 42, kind: 'text', text: 'Before the|warranty ends.', highlights: ['warranty'], size: 90, media: {src: 'newcar/p_dash.jpg', type: 'img', effect: 'in'}},
  {dur: 116, kind: 'text', text: 'Value you|never see leave.', highlights: ['never'], size: 90},
  {dur: 102, kind: 'text', enter: 'slideL', text: 'A 2–3-year-old car|is another deal.', highlights: ['deal'], size: 80, media: {src: 'newcar/p_usedlot.jpg', type: 'img', effect: 'in'}},
  {dur: 34, kind: 'text', text: 'It took the worst|of the fall.', highlights: ['worst'], size: 84, media: {src: 'newcar/v_used.mp4', type: 'video', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'And kept most|of its life.', highlights: ['life'], size: 88, media: {src: 'newcar/p_interior.jpg', type: 'img', effect: 'in'}},
  {dur: 55, kind: 'text', text: 'The first owner paid|for the new smell.', highlights: ['smell'], size: 82, media: {src: 'newcar/p_key.jpg', type: 'img', effect: 'in'}},
  {dur: 38, kind: 'text', text: "New isn't|a cost.", highlights: ['cost'], size: 96, media: {src: 'newcar/p_showroom.jpg', type: 'img', effect: 'in'}},
  {dur: 76, kind: 'text', enter: 'slideL', text: "It's a|luxury.", highlights: ['luxury'], size: 104, media: {src: 'newcar/v_luxury.mp4', type: 'video', effect: 'in'}},
  {dur: 80, kind: 'impact', text: 'The curve is|the real price tag.', redBg: true},
  {dur: 52, kind: 'text', text: 'Most vanishes|before year three.', highlights: ['vanishes'], size: 86},
  {dur: 63, kind: 'text', enter: 'zoom', text: 'who paid for|that new smell?', highlights: ['paid'], size: 84, media: {src: 'newcar/v_close.mp4', type: 'video', effect: 'in'}},
];
...
const SFX: SfxCue[] = [
  ...
  {at: 625, src: 'media/sfx_chaching.mp3', vol: 0.5},  // $8K year-one depreciation stat
  {at: 882, src: 'media/sfx_impact.mp3', vol: 0.5},    // 20% down stat
  {at: 955, src: 'media/sfx_impact.mp3', vol: 0.62},   // 60% down stat
  {at: 1197, src: 'media/sfx_whoosh.mp3', vol: 0.35},  // the used-car turn
  {at: 1549, src: 'media/sfx_impact.mp3', vol: 0.72},  // red 'real price tag' card
  {at: 1681, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // question close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The new-car tax</span>
      </div>
...
// ---------------------------------------------------------------------------
export const NewCarReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('newcar/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('newcar/music.mp3')}
