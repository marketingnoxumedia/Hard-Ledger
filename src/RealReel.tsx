...
// ---------------------------------------------------------------------------
// "Nominal vs real" reel (RealReel) — the QUESTION-hook A/B twin of SafeReel:
// same idle-cash-vs-inflation case (a ~0.5% account with ~3% inflation loses
// about 2.5% of its buying power a year, a negative REAL return, with nobody
// withdrawing a cent; because the number never drops the loss is invisible and
// compounds; nominal safety isn't real safety; cash guards a dip but exposes you
// to inflation — right for money you need soon, costly for money you keep for
// years), but this cut opens on a challenge hook ("You think cash is risk-free.
// So why…?") with the 0.5% / 3% / 2.5% stats landing inside the hook question,
// and closes on a question ("What is 'safe' money quietly costing you?").
// Durations on exact ElevenLabs spoken-word timestamps at +6% pace; the pause is
// spliced into the VO right after the hook question ("…buying power every year?")
// and beat 3 (the 2.5% stat) holds through it (77 -> 113). Total 1519 frames =
// ~50.6s. **Media-dense:** 19 of 22 beats carry footage, with only 2 flat-black
// beats ("A negative real return.", "Because the number holds.") plus the one
// red card; the close lands on footage. Own footage under public/real/ — no
// asset shared with SafeReel or any other reel (skill hard rule). **Illustrative
// figures — verify before publishing:** the 0.5% yield / 3% inflation / ~2.5%
// real-loss example is round-number arithmetic on stated assumptions, not a claim
// about any specific account or period; real cash returns vary by year. Same
// idle-cash-vs-inflation theme as SafeReel, MattressReel, and InflationReel —
// publish only one of the four to a given audience.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  ...
  {dur: 61, kind: 'text', text: 'Cash earns less|than inflation.', highlights: ['inflation'], size: 78, media: {src: 'real/v_shopping.mp4', type: 'video', effect: 'in'}},
  {dur: 35, kind: 'text', text: 'A negative|real return.', highlights: ['negative'], size: 88},
  {dur: 95, kind: 'text', text: 'The dollars stay.|What they buy shrinks.', highlights: ['shrinks'], size: 76, media: {src: 'real/p_dissolve.jpg', type: 'img', effect: 'in'}},
  {dur: 96, kind: 'text', enter: 'slideL', text: 'Nominal safety|isn\'t real safety.', highlights: ['real'], size: 78, media: {src: 'real/p_reflection.jpg', type: 'img', effect: 'in'}},
  {dur: 37, kind: 'text', text: 'Because the|number holds.', highlights: ['holds'], size: 86},
  {dur: 39, kind: 'text', text: 'So the loss|is invisible.', highlights: ['invisible'], size: 84, media: {src: 'real/p_hidden.jpg', type: 'img', effect: 'in'}},
  {dur: 89, kind: 'lines', text: 'No bad day.|No red figure.|Nothing to notice.', highlights: ['red'], reveal: [2, 32, 62], media: {src: 'real/p_flatgreen.jpg', type: 'img', effect: 'in'}},
  {dur: 77, kind: 'text', text: 'Purchasing power,|eroding.', highlights: ['eroding'], size: 82, media: {src: 'real/p_hourglass.jpg', type: 'img', effect: 'in'}},
  {dur: 58, kind: 'text', text: 'Compounding into|a real loss.', highlights: ['loss'], size: 82, media: {src: 'real/p_stairsdown.jpg', type: 'img', effect: 'in'}},
  {dur: 70, kind: 'impact', text: 'Safe.|Not risk-free.', redBg: true},
  {dur: 122, kind: 'text', enter: 'slideL', text: 'Guards a dip.|Exposes you to inflation.', highlights: ['exposes'], size: 74, media: {src: 'real/p_shield.jpg', type: 'img', effect: 'in'}},
  {dur: 60, kind: 'text', text: 'Right for money|you need soon.', highlights: ['soon'], size: 80, media: {src: 'real/p_purse.jpg', type: 'img', effect: 'in'}},
  {dur: 43, kind: 'text', text: 'Costly for money|you keep for years.', highlights: ['costly'], size: 76, media: {src: 'real/p_yearplanner.jpg', type: 'img', effect: 'in'}},
  {dur: 87, kind: 'text', text: 'The balance holds.|Buying power slips.', highlights: ['slips'], size: 78, media: {src: 'real/p_declineline.jpg', type: 'img', effect: 'in'}},
  {dur: 67, kind: 'text', enter: 'zoom', text: 'What is ‘safe’ money|quietly costing you?', highlights: ['costing'], size: 76, media: {src: 'real/v_thinking.mp4', type: 'video', effect: 'in'}},
];
...
// 188); the two cues before it sit on the pre-pause hook stats.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 84, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 188, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 483, src: 'media/sfx_impact.mp3', vol: 0.45},
  {at: 846, src: 'media/sfx_impact.mp3', vol: 0.45},
  {at: 1070, src: 'media/sfx_impact.mp3', vol: 0.6},
  {at: 1452, src: 'media/sfx_whoosh.mp3', vol: 0.4},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Nominal vs real</span>
      </div>
...
export const RealReel: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('real/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('real/music.mp3')}
