...
// ---------------------------------------------------------------------------
// "What's behind the hour" reel — the WHY behind the productivity gap (a distinct
// take from the PerHour/Hardest hours-vs-output reels: those show the correlation,
// this one explains the mechanism). Germany works ~1,300 hours a year, Mexico >2,200
// — but that doesn't mean Germans work harder or others work worse. Productivity
// isn't mainly effort; it's what sits behind each worker: machinery, technology,
// capital, infrastructure, logistics, management. One worker has a shovel, one has an
// excavator — who moves more dirt? Economies get rich not from more hours but from
// making every hour worth more. A punchy ~38s cut: two count-up stats (1,300 / 2,200),
// two list beats, the shovel-vs-excavator example (the memorable core), three black
...
// publishing. Imagery is kept neutral (no flags / no brand logos on the machinery);
// countries are named only in the VO and stat labels.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  ...
  {dur: 73, kind: 'text', text: 'Or others|work worse.', highlights: ['worse'], size: 100},
  {dur: 58, kind: 'text', text: 'It isn\'t mainly|about effort.', highlights: ['effort'], size: 92},
  {dur: 56, kind: 'text', enter: 'slideL', text: 'It\'s what sits behind|each worker.', highlights: ['behind'], size: 84, media: {src: 'capital/p_operator.jpg', type: 'img', effect: 'in'}},
  {dur: 64, kind: 'lines', text: 'Machinery.|Technology.|Capital.', highlights: ['capital'], media: {src: 'capital/p_machines.jpg', type: 'img', effect: 'in'}},
  {dur: 85, kind: 'lines', text: 'Infrastructure.|Logistics.|Management.', highlights: ['logistics'], media: {src: 'capital/p_logistics.jpg', type: 'img', effect: 'in'}},
  {dur: 32, kind: 'text', text: 'One worker|has a shovel.', highlights: ['shovel'], size: 92, media: {src: 'capital/p_shovel.jpg', type: 'img', effect: 'in'}},
  {dur: 39, kind: 'text', enter: 'slideL', text: 'One has|an excavator.', highlights: ['excavator'], size: 96, media: {src: 'capital/v_excavator.mp4', type: 'video', effect: 'in'}},
  {dur: 63, kind: 'text', text: 'Who moves|more dirt?', highlights: ['more'], size: 100, media: {src: 'capital/p_earthworks.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'text', text: 'Economies don\'t get rich|from more hours.', highlights: ['hours'], size: 84, media: {src: 'capital/p_growth.jpg', type: 'img', effect: 'in'}},
  {dur: 68, kind: 'impact', text: 'They get rich from|more per hour.', redBg: true},
  {dur: 75, kind: 'text', enter: 'slideL', text: 'The question was never|working more.', highlights: ['more'], size: 84, media: {src: 'capital/p_think.jpg', type: 'img', effect: 'in'}},
  {dur: 39, kind: 'text', text: 'Maybe it\'s|this...', highlights: ['this'], size: 104},
  {dur: 45, kind: 'text', enter: 'zoom', text: 'Why isn\'t each hour|worth more?', highlights: ['worth'], size: 88, media: {src: 'capital/v_close.mp4', type: 'video', effect: 'in'}},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>What's behind the hour</span>
      </div>
...
// ---------------------------------------------------------------------------
export const CapitalReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('capital/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('capital/music.mp3')}
