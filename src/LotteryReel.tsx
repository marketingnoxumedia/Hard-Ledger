...
// ---------------------------------------------------------------------------
// "Wealth is a habit" reel — the lottery-bankruptcy twist. The oft-repeated
// "most winners go broke" stat has no real source; the finding that does is
// stranger — a big win doesn't stop bankruptcy, it reschedules it. Researchers
// tracked lottery winners for five years: big winners were about 50% more likely
// to file than smaller winners, but not right away. A large prize delays
// bankruptcy by a few years rather than preventing it — the filings came three to
// five years after the win, and about 1 in 18 big winners filed within five
// years. A windfall lifts spending immediately; the skill to hold wealth takes
// longer to build than the money takes to spend. A lump sum is income; wealth is
// a habit — more money doesn't fix money habits, it amplifies them. The voiceover
// runs continuously with no dead air and scene durations sit on its exact
// spoken-word timestamps (ElevenLabs alignment) at +6% pace — total 1494 frames =
// ~50s. Mixes two video clips (spinning lottery tickets, a man in a sports car)
// with stills (a stack of $100s, an hourglass, an empty wallet, a wall calendar);
// two stats (50% / 1-in-18), the "wealth is a habit" red impact card mid-tail,
// and a "postponed, not prevented" calendar closer. Every background is sourced
// fresh for this reel (hard rule).
//
// NOTE (unverified): this reel cites a real academic finding on lottery winners
// and bankruptcy (large winners ~50% more likely to file than small winners;
// filings clustered ~3–5 years post-win; ~1 in 18 big winners filed within five
// years — figures associated with Hankins, Hoekstra & Skiba, "The Ticket to Easy
// Street?"). Rendered as written per the client script and NOT independently
// verified; confirm the study and figures before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 89, kind: 'hook', text: 'Most winners|go broke?', kicker: 'Wealth is a habit', highlights: ['broke'], size: 84, media: {src: 'lottery/clip_lotto.mp4', type: 'video', effect: 'in'}},
  {dur: 34, kind: 'text', text: 'No real|source.', highlights: ['source'], size: 92},
  {dur: 84, kind: 'text', text: 'The one that does|is stranger.', highlights: ['stranger'], size: 80},
  {dur: 41, kind: 'text', text: 'A big win doesn\'t|stop it.', highlights: ['stop'], size: 82},
  {dur: 46, kind: 'text', text: 'It reschedules|it.', highlights: ['reschedules'], size: 92},
  {dur: 98, kind: 'text', text: 'Researchers tracked|winners for five years.', highlights: ['tracked'], size: 70},
  {dur: 120, kind: 'stat', stat: {pre: 'Big winners, more likely to file', value: 50, suffix: '%'}},
  {dur: 127, kind: 'text', enter: 'zoom', text: 'The sum arrives faster|than the habits.', highlights: ['faster'], size: 72, media: {src: 'lottery/p_cash.jpg', type: 'img', effect: 'in'}},
  {dur: 36, kind: 'text', text: 'It doesn\'t|prevent it.', highlights: ['prevent'], size: 84},
  {dur: 67, kind: 'text', text: 'It only|delays it.', highlights: ['delays'], size: 88},
  {dur: 79, kind: 'text', enter: 'slideL', text: 'The money|buys time.', highlights: ['time'], size: 86, media: {src: 'lottery/p_hourglass.jpg', type: 'img', effect: 'in'}},
  {dur: 92, kind: 'text', text: 'Three to five years|after the win.', highlights: ['after'], size: 76},
  {dur: 88, kind: 'stat', stat: {pre: 'About 1 in', value: 18, post: 'big winners · filed in 5 yrs'}},
  {dur: 89, kind: 'text', enter: 'slideR', text: 'A windfall lifts|spending now.', highlights: ['spending'], size: 78, media: {src: 'lottery/clip_car.mp4', type: 'video', effect: 'in'}},
  {dur: 71, kind: 'text', text: 'Holding wealth|takes longer.', highlights: ['longer'], size: 80},
  {dur: 45, kind: 'text', text: 'A lump sum|is income.', highlights: ['income'], size: 86},
  {dur: 89, kind: 'impact', text: 'Wealth is|a habit.', redBg: true},
  {dur: 59, kind: 'text', enter: 'zoom', text: 'A bigger life|outlives the cash.', highlights: ['outlives'], size: 76, media: {src: 'lottery/p_wallet.jpg', type: 'img', effect: 'in'}},
  {dur: 37, kind: 'text', text: 'More money won\'t|fix habits.', highlights: ['fix'], size: 78},
  {dur: 27, kind: 'text', text: 'It amplifies|them.', highlights: ['amplifies'], size: 88},
  {dur: 25, kind: 'text', text: 'Not|prevented.', highlights: ['prevented'], size: 96},
  {dur: 51, kind: 'text', enter: 'zoom', text: 'Postponed.|A few years.', highlights: ['postponed'], size: 84, media: {src: 'lottery/p_calendar.jpg', type: 'img', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 248, src: 'media/sfx_impact.mp3', vol: 0.5}, // reschedules it
  {at: 294, src: 'media/sfx_whoosh.mp3', vol: 0.35}, // into the study
  {at: 392, src: 'media/sfx_impact.mp3', vol: 0.55}, // 50%
  {at: 913, src: 'media/sfx_impact.mp3', vol: 0.55}, // 1 in 18
  {at: 1001, src: 'media/sfx_whoosh.mp3', vol: 0.4}, // windfall spending
  {at: 1007, src: 'media/sfx_chaching.mp3', vol: 0.45},
  {at: 1206, src: 'media/sfx_impact.mp3', vol: 0.62}, // wealth is a habit
  {at: 1443, src: 'media/sfx_impact.mp3', vol: 0.5}, // closer
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Wealth is a habit</span>
      </div>
...
// ---------------------------------------------------------------------------
export const LotteryReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('lottery/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('lottery/music.mp3')}
