...
// ---------------------------------------------------------------------------
// "No bell" reel (BellReel) — an A/B TWIN of ClarityReel off the same news
// event (the Clarity Act dying, Sept 2026), but a different angle: instead of
// the CFTC/perps story, this is the SEC's 24-hour-trading / "no bell" angle.
// Thesis: Wall Street doesn't need Congress to change the rules — it just needs
// the market to stop closing. The stock market runs on one bell, 9:30–4:00,
// weekdays only — 6.5 hours a day, the other 17.5 locked. The Senate killed the
// Clarity Act; eight days later the SEC granted an innovation exemption putting
// tokenised US stocks on public blockchains — and a blockchain has one thing the
// NYSE doesn't: no bell. The catch: it's an exemption, not a law (5 years, then
// back on the table). A two-bar chart contrasts the exchange's 6.5h against the
// chain's 24h (~3.7× the trading window). Payoff: the winner is whoever owns the
// rails that never switch off. Close: the bell still rings at four — it just
// doesn't mean anything anymore. Like ClarityReel, this NEWS reel departs from
// two house conventions by client direction: a hard-cut DECLARATIVE hook (no
// withheld-subject question, so no post-hook pause) and a non-question CTA close
// ("Keep your eyes open."). Uses the per-reel UPBEAT cloned voice (same as
// ClarityReel), not the shared house voice. Durations on exact ElevenLabs spoken
// timestamps at +6% pace (no pause splice); total 1653 frames = ~55.1s. Adds a
// two-bar `chart` scene kind (the client asked for the chart explicitly). Media
// -dense: 17 of 20 beats carry footage, 2 flat-black beats + 1 red card. Own
// footage under public/bell/ — no asset shared with any other reel (skill hard
// rule). **TIME-SENSITIVE / UNVERIFIED — verify before publishing:** the Clarity
// Act Senate vote and its Sept-2026 date, the "SEC innovation exemption / tokenised
// US stocks / 5-year, zero-vote" moves, and the 6.5h-vs-24h / ~3.7× framing are
// dated news supplied by the author; rendered as written and to be re-checked
// against primary sources at publish time.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 85, kind: 'hook', text: 'Wall Street doesn\'t|need Congress.', kicker: 'No bell', highlights: ['need'], size: 80, media: {src: 'bell/p_wallst.jpg', type: 'img', effect: 'in'}},
  {dur: 70, kind: 'text', text: 'It needs the market|to stop closing.', highlights: ['closing'], size: 80, media: {src: 'bell/p_afterhours.jpg', type: 'img', effect: 'in'}},
  {dur: 76, kind: 'lines', text: 'Every stock.|Every trade.|Every pension.', highlights: ['pension'], reveal: [0, 25, 51], media: {src: 'bell/v_ticker.mp4', type: 'video', effect: 'in'}},
  {dur: 107, kind: 'text', text: 'One bell.|9:30 to 4.', highlights: ['bell'], size: 84, media: {src: 'bell/p_bell.jpg', type: 'img', effect: 'in'}},
  {dur: 41, kind: 'stat', stat: {pre: 'Six and a half', value: 6.5, decimals: 1, post: 'HOURS A DAY'}, media: {src: 'bell/p_clock.jpg', type: 'img', effect: 'in'}},
  {dur: 99, kind: 'text', text: 'The other 17.|Locked.', highlights: ['locked'], size: 84},
  {dur: 97, kind: 'text', enter: 'slideL', text: 'September 2026.|The Senate: no.', highlights: ['no'], size: 74, media: {src: 'bell/p_capitol2.jpg', type: 'img', effect: 'in'}},
  {dur: 69, kind: 'text', text: '8 days later.|The SEC moves.', highlights: ['moves'], size: 80, media: {src: 'bell/p_sec.jpg', type: 'img', effect: 'in'}},
  {dur: 37, kind: 'text', text: 'An innovation|exemption.', highlights: ['exemption'], size: 84, media: {src: 'bell/p_exempt.jpg', type: 'img', effect: 'in'}},
  {dur: 164, kind: 'lines', text: 'Tokenized stocks.|Public chains.|Zero votes.', highlights: ['zero'], reveal: [0, 60, 126], media: {src: 'bell/p_chain.jpg', type: 'img', effect: 'in'}},
  {dur: 113, kind: 'text', text: 'One thing the NYSE|doesn\'t have.', highlights: ['nyse'], size: 72, media: {src: 'bell/v_floor.mp4', type: 'video', effect: 'in'}},
  {dur: 35, kind: 'impact', text: 'No|bell.', redBg: true},
  {dur: 43, kind: 'text', text: 'An exemption.|Not a law.', highlights: ['law'], size: 84},
  {dur: 64, kind: 'text', text: 'Five years.|Then back.', highlights: ['back'], size: 84, media: {src: 'bell/p_hourglass.jpg', type: 'img', effect: 'in'}},
  {dur: 41, kind: 'text', enter: 'slideL', text: 'The rails|moved first.', highlights: ['first'], size: 84, media: {src: 'bell/v_rails.mp4', type: 'video', effect: 'in'}},
  {dur: 127, kind: 'chart', chart: {a: {label: 'NYSE', value: 6.5}, b: {label: 'CHAIN', value: 24, red: true}, unit: 'HRS / DAY', decimals: 1}, media: {src: 'bell/p_serverroom.jpg', type: 'img', effect: 'in'}},
  {dur: 88, kind: 'stat', stat: {pre: 'Same stock', value: 3.7, decimals: 1, suffix: 'x', post: 'THE TRADING WINDOW'}, media: {src: 'bell/p_uptrend.jpg', type: 'img', effect: 'in'}},
  {dur: 147, kind: 'text', text: 'Not the trader.|Whoever owns the rails.', highlights: ['rails'], size: 74, media: {src: 'bell/p_cables.jpg', type: 'img', effect: 'in'}},
  {dur: 45, kind: 'text', text: 'The bell still rings|at four.', highlights: ['bell'], size: 76, media: {src: 'bell/p_bell2.jpg', type: 'img', effect: 'in'}},
  {dur: 105, kind: 'text', enter: 'zoom', text: 'It means nothing now.|Keep your eyes open.', highlights: ['open'], size: 74, media: {src: 'bell/v_night.mp4', type: 'video', effect: 'in'}},
];
...
const SFX: SfxCue[] = [
  ...
  {at: 958, src: 'media/sfx_impact.mp3', vol: 0.6},
  {at: 1141, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 1268, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1548, src: 'media/sfx_whoosh.mp3', vol: 0.4},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>No bell</span>
      </div>
...
// ---------------------------------------------------------------------------
export const BellReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('bell/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('bell/music.mp3')}
