...
// ---------------------------------------------------------------------------
// "The no-interest loan" reel — the tax refund is your own money, returned late
// with nothing added. Durations placed on exact spoken-word timestamps
// (ElevenLabs alignment) at +6% pace. Total 1245 frames = ~41.5s. Ends on a
// media beat (a wall clock), not the red card — the red `impact` sits at the
// penultimate thesis beat. Every scene uses a distinct background sourced fresh
// for this reel — no footage shared with any other reel (see the hard rule).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 76, kind: 'hook', text: 'The average American|gets a tax refund…', kicker: 'The refund trap', highlights: ['refund'], size: 78, media: {src: 'refund/p_7688995.jpg', type: 'img', effect: 'in'}},
  {dur: 106, kind: 'stat', stat: {pre: 'The average refund', prefix: '$', value: 3300, post: 'treated like a windfall'}, media: {src: 'refund/p_3531895.jpg', type: 'img', effect: 'in'}},
  {dur: 23, kind: 'text', text: 'It isn\'t.', highlights: ['isn\'t'], size: 120},
  {dur: 27, kind: 'text', text: 'It\'s your|own money.', highlights: ['your'], size: 96, media: {src: 'refund/p_545065.jpg', type: 'img', effect: 'in'}},
  {dur: 90, kind: 'text', text: 'Returned late.|Nothing added.', highlights: ['late'], size: 84, media: {src: 'refund/p_8102178.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'text', enter: 'slideL', text: 'That refund means|you overpaid.', highlights: ['overpaid'], size: 80, media: {src: 'refund/p_6694543.jpg', type: 'img', effect: 'in'}},
  {dur: 76, kind: 'stat', stat: {pre: 'You overpaid', prefix: '$', value: 275, post: 'every single month'}, media: {src: 'refund/p_9755376.jpg', type: 'img', effect: 'in'}},
  {dur: 30, kind: 'text', text: 'All year|long.', highlights: ['long'], size: 100},
  {dur: 51, kind: 'text', enter: 'slideR', text: 'Money the government|held. And used.', highlights: ['used'], size: 78, media: {src: 'refund/p_12505005.jpg', type: 'img', effect: 'in'}},
  {dur: 99, kind: 'text', text: 'Then gave it back|interest free.', highlights: ['free'], size: 78, media: {src: 'refund/p_1550334.jpg', type: 'img', effect: 'panL'}},
  {dur: 63, kind: 'text', enter: 'zoom', text: 'That same money,|kept and invested.', highlights: ['invested'], size: 78, media: {src: 'refund/p_7567223.jpg', type: 'img', effect: 'in'}},
  {dur: 57, kind: 'stat', stat: {pre: 'Compounded at', value: 7, suffix: '%', post: 'a year'}, media: {src: 'refund/p_6770610.jpg', type: 'img', effect: 'in'}},
  {dur: 109, kind: 'stat', stat: {pre: 'Invested instead', prefix: '$', value: 335, suffix: 'K', post: 'over a career'}},
  {dur: 44, kind: 'text', text: 'Instead of one cheque|every spring.', highlights: ['cheque'], size: 78, media: {src: 'refund/p_6862457.jpg', type: 'img', effect: 'in'}},
  {dur: 95, kind: 'text', enter: 'slideL', text: 'A big refund feels|like a win.', highlights: ['win'], size: 80, media: {src: 'refund/p_7680634.jpg', type: 'img', effect: 'in'}},
  {dur: 33, kind: 'text', text: 'Because it doesn\'t feel|like your money.', highlights: ['money'], size: 74, media: {src: 'refund/p_6590651.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', text: 'The refund|isn\'t a gift.', highlights: ['gift'], size: 90, media: {src: 'refund/p_1661959.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'impact', text: 'A no-interest loan|you didn\'t mean to make.', redBg: true},
  {dur: 81, kind: 'text', enter: 'zoom', text: 'Money returned late is|money that never|got to work.', highlights: ['never'], size: 72, media: {src: 'refund/p_191703.jpg', type: 'img', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 76, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 182, src: 'media/sfx_whoosh.mp3', vol: 0.45},
  {at: 385, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 491, src: 'media/sfx_whoosh.mp3', vol: 0.45},
  {at: 704, src: 'media/sfx_impact.mp3', vol: 0.42},
  {at: 761, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 914, src: 'media/sfx_impact.mp3', vol: 0.4},
  {at: 1091, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1164, src: 'media/sfx_whoosh.mp3', vol: 0.45},
];
...
const SceneOutro: React.FC = () => {
  ...
  const {fps} = useVideoConfig();
  const parts = [
    {t: 'Not a gift.', red: false},
    {t: 'Not a bonus.', red: false},
    {t: 'Just late.', red: true},
  ];
  return (
    <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
        ...
        {parts.map((p, i) => {
          ...
          return (
            <span key={i} style={{fontFamily: HEAD, fontSize: p.red ? 150 : 116, textTransform: 'uppercase', letterSpacing: 1, color: C.ink, opacity: s, transform: `translateY(${interpolate(s, [0, 1], [34, 0])}px)`}}>
              {p.red ? (<>Just <span style={{color: C.red, textShadow: SH}}>late.</span></>) : p.t}
            </span>
          );
        })}
      </div>
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The refund trap</span>
      </div>
...
// ---------------------------------------------------------------------------
export const RefundReel: React.FC = () => {
  const frame = useCurrentFrame();
