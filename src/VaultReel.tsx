...
// ---------------------------------------------------------------------------
// "Safe vs growing" reel (VaultReel) — gold vs stocks as a store of value, told as the A/B twin of
// the existing GoldReel ("Store of value"), which opens and closes declaratively. Here the hook and
// close are rhetorical questions. The asset people trust most to protect wealth is one of the worst
// at growing it: since 1926, a dollar in gold is worth about $5 today (after inflation) while the
// same dollar in stocks is worth about $1,081. Gold feels safe — tangible, ancient, climbs in the
// crises people remember — but both figures are real, and gold roughly held its value while stocks
// multiplied it hundreds of times. It isn't one lucky stretch: gold earns nothing (no dividend; its
// only return is what the next buyer pays), while stocks compound because the businesses underneath
// them reinvest and grow. Safe and growing are two different jobs; confusing them is the quiet,
// expensive mistake — over decades a store of value that barely grows just falls behind. Question
// hook ("...one of the worst at growing it?") and a rhetorical question close ("for a 30-year goal:
// something that sits, or something that compounds?"); a ~1.2s post-hook pause (the hook holds
// through the silence before the reveal — house convention); two count-up `stat` beats ($5 gold vs
// $1,081 stocks), one red "Confusing them is expensive." `impact` card as the mid-tail thesis, and
// the question close on footage. **Media-dense:** 20 of 23 beats carry footage, with only 2 flat-black
// beats ("Not one lucky stretch.", "just falls behind.") plus the one red card. Durations on exact
// ElevenLabs spoken-word timestamps at +6% pace; music normalized and mixed at 0.17. Total 1652
// frames = ~55.1s. Entirely its own fresh footage under public/vault/ — no asset shared with GoldReel
// or any other reel. **Brand-safety:** the reel names no company — footage is generic gold/finance
// imagery; chart/screen shots avoid a dominant readable company name or brokerage logo. Confirm no
// stray ticker reads as a single company before publishing. **Illustration / verify the figures:**
// the ~$5-vs-$1,081 (a 1926 dollar to today, real) contrast reflects widely-cited long-run return
// studies, but the exact endpoints depend on the index, dates, inflation series and assumptions —
// verify the specific numbers and cite the source before publishing; gold's real return in particular
...
const SCENES: SceneDef[] = [
  ...
  {dur: 108, kind: 'stat', enter: 'slideL', stat: {pre: 'A DOLLAR IN GOLD, SINCE 1926', prefix: '$', value: 5, post: 'TODAY'}, media: {src: 'vault/p_goldcoins.jpg', type: 'img', effect: 'in'}},
  {dur: 69, kind: 'stat', stat: {pre: 'THE SAME IN STOCKS', prefix: '$', value: 1081}, media: {src: 'vault/v_marketchart.mp4', type: 'video', effect: 'in'}},
  ...
  {dur: 42, kind: 'text', text: 'Not one|lucky stretch.', highlights: ['lucky'], size: 92},
  {dur: 103, kind: 'text', text: 'Gold earns|nothing.', highlights: ['nothing'], size: 96, media: {src: 'vault/p_goldpile.jpg', type: 'img', effect: 'in'}},
  {dur: 33, kind: 'text', text: 'Its return: the|next buyer.', highlights: ['buyer'], size: 84, media: {src: 'vault/p_buyer.jpg', type: 'img', effect: 'in'}},
  {dur: 86, kind: 'text', enter: 'slideL', text: 'Stocks|compound.', highlights: ['compound'], size: 104, media: {src: 'vault/v_compound.mp4', type: 'video', effect: 'in'}},
  {dur: 39, kind: 'text', text: 'Businesses reinvest|and grow.', highlights: ['grow'], size: 84, media: {src: 'vault/p_business.jpg', type: 'img', effect: 'in'}},
...
const SFX: SfxCue[] = [
  {at: 222, src: 'media/sfx_whoosh.mp3', vol: 0.4},    // reveal after the pause ($5 gold stat)
  {at: 330, src: 'media/sfx_chaching.mp3', vol: 0.5},  // $1,081 stocks stat (the big number)
  {at: 708, src: 'media/sfx_impact.mp3', vol: 0.45},   // "Stocks: past a thousand"
  {at: 1025, src: 'media/sfx_whoosh.mp3', vol: 0.35},  // "Stocks compound"
  {at: 1294, src: 'media/sfx_impact.mp3', vol: 0.72},  // red 'Confusing them is expensive' card
  {at: 1568, src: 'media/sfx_whoosh.mp3', vol: 0.4},   // question close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Safe vs growing</span>
      </div>
...
// ---------------------------------------------------------------------------
export const VaultReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('vault/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('vault/music.mp3')}
