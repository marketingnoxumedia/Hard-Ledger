...
// ---------------------------------------------------------------------------
// "Rate beats salary" reel — two workers, same 30 years, same market. One earns
// $150K and saves 5%; the other earns $60K and saves 20%. So the higher earner
// invests $7,500 a year and the lower earner $12,000. Run both at an assumed 7%
// for 30 years and the pots flip: the higher earner reaches about $0.7M, the
// lower earner about $1.1M. The one who put in more each year wins, regardless of
// who earned more — the salary set the ceiling, the savings rate decided the
// outcome, and a raise you spend changes nothing. Net worth tracks the gap
// between earning and spending, not the size of the paycheck. The voiceover runs
// continuously with no dead air and scene durations sit on its exact spoken-word
// timestamps (ElevenLabs alignment) at +6% pace — total 1361 frames = ~45s.
// Mixes two video clips (two colleagues at a laptop, a retired couple walking)
// with stills (a market ticker, a seaside bench, stacked coins, a glass tower,
// shopping bags, a coin jar sprouting); single-figure stats ($150K / $60K / 5% /
// 20% / 7%), two contrast compares ($7,500 vs $12,000 per year, $0.7M vs $1.1M
// pots), the "rate beats salary" red impact card mid-tail, and an earned-less /
// saved-more / retired-richer lines closer. Every background is sourced fresh for
// this reel (hard rule).
//
// NOTE (illustration): the figures are a worked example, not real people — two
// hypothetical savers compounded at an assumed 7% for 30 years ($150K×5% =
// $7,500/yr ≈ $0.7M; $60K×20% = $12,000/yr ≈ $1.1M). Rendered as written; the
// 7% return and round pot figures are illustrative and not a forecast.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 25, kind: 'hook', text: 'Two|workers.', kicker: 'Rate beats salary', highlights: ['two'], size: 104, media: {src: 'saver/clip_workers.mp4', type: 'video', effect: 'in'}},
  {dur: 55, kind: 'text', text: 'Same thirty years.|Same market.', highlights: ['same'], size: 76, media: {src: 'saver/p_market.jpg', type: 'img', effect: 'in'}},
  {dur: 58, kind: 'stat', stat: {pre: 'One earns', prefix: '$', value: 150, suffix: 'K'}},
  {dur: 92, kind: 'stat', stat: {pre: 'The other earns', prefix: '$', value: 60, suffix: 'K'}},
  {dur: 60, kind: 'text', enter: 'zoom', text: 'Earned less —|retired with more.', highlights: ['more'], size: 76, media: {src: 'saver/p_retire.jpg', type: 'img', effect: 'in'}},
  {dur: 83, kind: 'stat', stat: {pre: 'The $150K earner saves', value: 5, suffix: '%'}},
  {dur: 86, kind: 'stat', stat: {pre: 'The $60K earner saves', value: 20, suffix: '%'}},
  {dur: 142, kind: 'compare', compare: {a: {value: 7500, prefix: '$', label: 'higher earner'}, b: {value: 12000, prefix: '$', label: 'lower earner'}, note: 'Invested per year'}},
  {dur: 105, kind: 'stat', stat: {pre: 'Assumed return', value: 7, suffix: '%', post: 'for 30 years · illustration'}},
  {dur: 143, kind: 'compare', compare: {a: {value: 0.7, decimals: 1, prefix: '$', suffix: 'M', label: 'higher earner'}, b: {value: 1.1, decimals: 1, prefix: '$', suffix: 'M', label: 'lower earner'}, note: 'Same 7%, same 30 years'}},
  {dur: 50, kind: 'text', enter: 'slideL', text: 'Put in more|each year — wins.', highlights: ['wins'], size: 78, media: {src: 'saver/p_save.jpg', type: 'img', effect: 'in'}},
  {dur: 48, kind: 'text', text: 'Regardless of who|earned more.', highlights: ['earned'], size: 76},
  {dur: 44, kind: 'text', text: 'The salary set|the ceiling.', highlights: ['ceiling'], size: 80, media: {src: 'saver/p_ceiling.jpg', type: 'img', effect: 'in'}},
  {dur: 61, kind: 'text', text: 'The savings rate|decided it.', highlights: ['rate'], size: 82},
  {dur: 65, kind: 'text', enter: 'slideR', text: 'A raise you spend|changes nothing.', highlights: ['nothing'], size: 76, media: {src: 'saver/p_spend.jpg', type: 'img', effect: 'in'}},
...
const SFX: SfxCue[] = [
  {at: 80, src: 'media/sfx_impact.mp3', vol: 0.5}, // $150K
  {at: 138, src: 'media/sfx_impact.mp3', vol: 0.5}, // $60K
  {at: 373, src: 'media/sfx_impact.mp3', vol: 0.5}, // 20% save rate
  {at: 459, src: 'media/sfx_whoosh.mp3', vol: 0.4}, // invested compare
  {at: 706, src: 'media/sfx_impact.mp3', vol: 0.62}, // pot payoff compare
  {at: 712, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1117, src: 'media/sfx_impact.mp3', vol: 0.62}, // rate beats salary
  {at: 1274, src: 'media/sfx_impact.mp3', vol: 0.45}, // closer
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Rate beats salary</span>
      </div>
...
// ---------------------------------------------------------------------------
export const SaverReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('saver/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('saver/music.mp3')}
