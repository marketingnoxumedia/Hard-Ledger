...
type CompareCfg = {
  a: {value: number; prefix?: string; suffix?: string; label: string};
  b: {value: number; prefix?: string; suffix?: string; label: string};
  note?: string;
  source?: string;
...
// ---------------------------------------------------------------------------
// "Looks like a toy" reel — the long-form Blockbuster/Netflix story. In 2000,
// Blockbuster could have bought Netflix for about $50M and laughed it off. On
// the numbers of the day, refusing looked sound: the stores were profitable, the
// brand was national, late fees were dependable — and Netflix was tiny,
// unprofitable, and dependent on the mail. Buying it also meant funding a model
// whose success would kill the very late fees paying the bills. Streaming
// arrived, late fees died, customers left, and Blockbuster filed for bankruptcy
// in 2010. Netflix went on to be worth over $400B — roughly 8,000× the price
// Blockbuster wouldn't pay. The danger wasn't a bigger rival; it was a tiny one
// solving the customer's real annoyance — the thing that looks like a toy is
// often the thing that ends you. The voiceover runs continuously with no dead
// air and scene durations sit on its exact spoken-word timestamps (ElevenLabs
// alignment) at +6% pace — total 1795 frames = ~60s. Mixes two video clips (a
// video-store browse, a screen-lit viewer) with stills (a CRT TV, a long
// receipt, a held disc, a neon rental store, a shuttered storefront); three
// stats ($50M / $400B / 8,000×), the "looks like a toy" red impact card, and a
// closing $50M-refused vs $400B-gone comparison. Every background is sourced
// fresh for this reel (hard rule).
//
// NOTE (unverified): this reel names real companies (Blockbuster, Netflix) and
// cites a widely-reported anecdote — the ~$50M Netflix offer Blockbuster
// declined circa 2000, Blockbuster's 2010 bankruptcy, Netflix later worth over
// $400B, and the resulting ~8,000× gap. Rendered as written per the client
// script and NOT independently verified; confirm the figures before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 63, kind: 'hook', text: 'One company|could\'ve bought it.', kicker: 'Looks like a toy', highlights: ['bought'], size: 78, media: {src: 'toy/p_tv.jpg', type: 'img', effect: 'in'}},
  {dur: 43, kind: 'text', text: 'The thing that|would destroy it.', highlights: ['destroy'], size: 76},
  {dur: 59, kind: 'stat', stat: {pre: 'The asking price', prefix: '$', value: 50, suffix: 'M'}},
  {dur: 47, kind: 'text', text: 'It laughed the|offer away.', highlights: ['laughed'], size: 82},
  {dur: 93, kind: 'text', enter: 'zoom', text: 'It was|Blockbuster.', highlights: ['blockbuster'], size: 92, media: {src: 'toy/clip_store.mp4', type: 'video', effect: 'in'}},
  {dur: 73, kind: 'text', text: 'A fortune|in late fees.', highlights: ['fees'], size: 84, media: {src: 'toy/p_receipt.jpg', type: 'img', effect: 'in'}},
  {dur: 55, kind: 'text', text: 'The customer\'s|biggest grievance.', highlights: ['grievance'], size: 74},
  {dur: 86, kind: 'text', enter: 'slideL', text: 'A DVD-by-mail|startup.', highlights: ['startup'], size: 80, media: {src: 'toy/p_disc.jpg', type: 'img', effect: 'in'}},
  {dur: 108, kind: 'text', text: 'It offered to|sell itself.', highlights: ['sell'], size: 82},
  {dur: 43, kind: 'text', text: 'Refusing|looked sound.', highlights: ['sound'], size: 84},
  {dur: 127, kind: 'text', enter: 'slideL', text: 'The stores|were profitable.', highlights: ['profitable'], size: 80, media: {src: 'toy/p_store.jpg', type: 'img', effect: 'in'}},
  {dur: 123, kind: 'lines', text: 'Tiny.|Unprofitable.|On the mail.', highlights: ['tiny'], reveal: [6, 46, 86]},
  {dur: 101, kind: 'text', enter: 'slideR', text: 'Buying it meant|paying real money.', highlights: ['money'], size: 78},
  {dur: 69, kind: 'text', text: 'To kill|its own fees.', highlights: ['kill'], size: 86},
  {dur: 114, kind: 'text', enter: 'zoom', text: 'Streaming|arrived.', highlights: ['streaming'], size: 96, media: {src: 'toy/clip_stream.mp4', type: 'video', effect: 'in'}},
  {dur: 113, kind: 'text', text: 'Bankruptcy.|2010.', highlights: ['2010'], size: 92, media: {src: 'toy/p_closed.jpg', type: 'img', effect: 'in'}},
  {dur: 66, kind: 'stat', stat: {pre: 'Netflix, later worth', prefix: '$', value: 400, suffix: 'B'}},
  {dur: 92, kind: 'stat', stat: {pre: 'Roughly', value: 8000, comma: true, suffix: '×', post: 'the price Blockbuster|wouldn\'t pay'}},
  {dur: 78, kind: 'text', text: 'The danger wasn\'t|a bigger rival.', highlights: ['rival'], size: 76},
  {dur: 73, kind: 'text', text: 'A tiny one solving|the annoyance.', highlights: ['solving'], size: 74},
  {dur: 72, kind: 'impact', text: 'Looks like|a toy.', redBg: true},
  {dur: 97, kind: 'compare', compare: {a: {value: 50, prefix: '$', suffix: 'M', label: 'refused'}, b: {value: 400, prefix: '$', suffix: 'B', label: 'gone'}, note: 'The toy that ended it', source: 'Blockbuster → Netflix'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 106, src: 'media/sfx_impact.mp3', vol: 0.5}, // $50M
  {at: 212, src: 'media/sfx_whoosh.mp3', vol: 0.38}, // cut into Blockbuster
  {at: 1090, src: 'media/sfx_whoosh.mp3', vol: 0.42}, // streaming arrives
  {at: 1204, src: 'media/sfx_impact.mp3', vol: 0.6}, // bankruptcy 2010
  {at: 1317, src: 'media/sfx_impact.mp3', vol: 0.55}, // $400B
  {at: 1323, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1383, src: 'media/sfx_impact.mp3', vol: 0.5}, // 8,000×
  {at: 1626, src: 'media/sfx_impact.mp3', vol: 0.62}, // looks like a toy
  {at: 1698, src: 'media/sfx_impact.mp3', vol: 0.5}, // closing compare
];
...
const SceneCompare: React.FC<{cfg: CompareCfg}> = ({cfg}) => {
  ...
  const fmt = (v: number, prefix?: string, suffix?: string) => (prefix ?? '') + Math.round(v * t).toLocaleString('en-US') + (suffix ?? '');
    ...
    <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center', padding: 70}}>
      <div style={{opacity: enter, transform: `translateY(${interpolate(enter, [0, 1], [28, 0])}px)`, display: 'flex', flexDirection: 'column', gap: 18, alignItems: 'center'}}>
        <div style={{display: 'flex', alignItems: 'baseline', gap: 26, justifyContent: 'center'}}>
          <span style={{fontFamily: HEAD, fontSize: 200, lineHeight: 0.9, color: C.ink, letterSpacing: 1, textShadow: SH}}>{fmt(cfg.a.value, cfg.a.prefix, cfg.a.suffix)}</span>
        ...
        <div style={{display: 'flex', alignItems: 'baseline', gap: 26, justifyContent: 'center'}}>
          <span style={{fontFamily: HEAD, fontSize: 250, lineHeight: 0.9, color: C.red, letterSpacing: 1, textShadow: SH}}>{fmt(cfg.b.value, cfg.b.prefix, cfg.b.suffix)}</span>
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Looks like a toy</span>
      </div>
...
export const ToyReel: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('toy/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('toy/music.mp3')}
