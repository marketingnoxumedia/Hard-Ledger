...
// ---------------------------------------------------------------------------
// "The everything store" reel — Sears invented mail-order retail a century
// before Amazon, then couldn't build the online version of itself because it
// would have undercut the stores that made the profit. Durations placed on
// exact spoken-word timestamps (ElevenLabs alignment) at +6% pace. Total 1287
// frames = ~42.9s. The red `impact` card ("bankrupt / 2018") sits near the end
// as the thesis punch; the reel closes on an abandoned-store media beat, not
...
const SCENES: SceneDef[] = [
  {dur: 60, kind: 'hook', text: 'It invented|mail-order shopping.', kicker: 'The everything store', highlights: ['invented'], size: 78, media: {src: 'sears/p_32122734.jpg', type: 'img', effect: 'in'}},
  {dur: 54, kind: 'text', text: 'A century|before Amazon.', highlights: ['century'], size: 84, media: {src: 'sears/p_17660925.jpg', type: 'img', effect: 'in'}},
  {dur: 85, kind: 'text', enter: 'slideL', text: 'The original|everything store.', highlights: ['everything'], size: 82, media: {src: 'sears/p_36963719.jpg', type: 'img', effect: 'in'}},
  {dur: 60, kind: 'text', text: 'Then it watched|someone build|the internet version.', highlights: ['internet'], size: 72, media: {src: 'sears/clip_online.mp4', type: 'video', effect: 'in'}},
  {dur: 28, kind: 'text', text: 'And went|bankrupt.', highlights: ['bankrupt'], size: 100},
  {dur: 54, kind: 'stat', stat: {pre: 'A retail titan for', value: 100, post: 'years'}, media: {src: 'sears/p_15382149.jpg', type: 'img', effect: 'in'}},
  {dur: 90, kind: 'text', text: 'Sears was|a retail titan.', highlights: ['titan'], size: 82, media: {src: 'sears/p_11909534.jpg', type: 'img', effect: 'panR'}},
  {dur: 65, kind: 'text', text: 'Buy almost|anything by mail.', highlights: ['anything'], size: 80, media: {src: 'sears/p_10669434.jpg', type: 'img', effect: 'in'}},
  {dur: 84, kind: 'text', enter: 'slideR', text: 'The failure is stranger|than complacency.', highlights: ['stranger'], size: 74, media: {src: 'sears/p_15044919.jpg', type: 'img', effect: 'in'}},
  {dur: 38, kind: 'text', text: 'Its catalog was|the ancestor…', highlights: ['ancestor'], size: 82, media: {src: 'sears/p_13081133.jpg', type: 'img', effect: 'in'}},
  {dur: 55, kind: 'text', text: '…of the model|Amazon would use.', highlights: ['Amazon'], size: 80, media: {src: 'sears/clip_conveyor.mp4', type: 'video', effect: 'in'}},
  {dur: 124, kind: 'text', enter: 'zoom', text: 'But the stores|were the profit.', highlights: ['profit'], size: 82, media: {src: 'sears/p_18197630.jpg', type: 'img', effect: 'panL'}},
  {dur: 84, kind: 'text', text: 'Going online meant|undercutting itself.', highlights: ['undercutting'], size: 74, media: {src: 'sears/clip_card.mp4', type: 'video', effect: 'in'}},
  {dur: 35, kind: 'text', text: 'Its whole|format.', highlights: ['whole'], size: 96},
  {dur: 59, kind: 'text', text: 'Nimbler|online rivals.', highlights: ['nimbler'], size: 86, media: {src: 'sears/p_8377802.jpg', type: 'img', effect: 'in'}},
  {dur: 71, kind: 'text', text: 'Captured the future|Sears invented.', highlights: ['future'], size: 78, media: {src: 'sears/clip_warehouse.mp4', type: 'video', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'Decades|of decline.', highlights: ['decline'], size: 92, media: {src: 'sears/p_4960603.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'impact', text: 'Bankrupt.|2018.', redBg: true},
  {dur: 82, kind: 'text', text: 'Inventing it once|is no defence…', highlights: ['once'], size: 76, media: {src: 'sears/p_34392837.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', enter: 'zoom', text: '…against missing|its next version.', highlights: ['next'], size: 78, media: {src: 'sears/p_9539134.jpg', type: 'img', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 114, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 259, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 287, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 618, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 916, src: 'media/sfx_whoosh.mp3', vol: 0.42},
  {at: 1093, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1238, src: 'media/sfx_whoosh.mp3', vol: 0.45},
];
...
const SceneOutro: React.FC = () => {
  ...
  const {fps} = useVideoConfig();
  const parts = [
    {t: 'Invented the model.', red: false},
    {t: 'Missed the sequel.', red: false},
    {t: 'Gone.', red: true},
  ];
  return (
    <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
        ...
        {parts.map((p, i) => {
          ...
          return (
            <span key={i} style={{fontFamily: HEAD, fontSize: p.red ? 150 : 116, textTransform: 'uppercase', letterSpacing: 1, color: C.ink, opacity: s, transform: `translateY(${interpolate(s, [0, 1], [34, 0])}px)`}}>
              {p.red ? (<span style={{color: C.red, textShadow: SH}}>Gone.</span>) : p.t}
            </span>
          );
        })}
      </div>
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The everything store</span>
      </div>
...
// ---------------------------------------------------------------------------
export const SearsReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('sears/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('sears/music.mp3')}
