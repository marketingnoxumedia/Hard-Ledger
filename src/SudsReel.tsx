...
// ---------------------------------------------------------------------------
// "A subscription with soap" reel — why Wall Street is buying car washes. It's a
// business you drive through in four minutes: no app, no founder, just soap, water
// and a monthly charge that never stops. The modern car wash stopped charging per
// wash and started selling unlimited monthly memberships, turning a one-off service
// into recurring, subscription revenue. What attracts the money isn't the wash,
// it's the contract behind it: members pay every month whether they show up or not,
// cost per wash is low, and the land underneath holds its value. A weather-
// dependent, one-off purchase becomes predictable monthly cash — exactly the
// cash-flow profile a financial buyer pays a premium for. So private equity has
// been rolling up independent washes into large chains, chasing that recurring
// revenue. The thing being bought is two assets in one purchase: the recurring
// membership base and the appreciating real estate under it. Recurring changes
// everything — attach a subscription to a piece of land and an unglamorous service
// becomes a roll-up target. The voiceover runs continuously with no dead air and
// scene durations sit on its exact spoken-word timestamps (ElevenLabs alignment) at
// +6% pace — total 1866 frames = ~62s. Full-bleed footage on nearly every beat,
// with one black beat (the "no app / no founder" list) and one red "recurring
// changes everything" impact card, closing on soapy footage. Every background is
// sourced fresh for this reel (hard rule). Distinct from the existing CarWashReel.
//
// NOTE (illustration, not figures): "Wall Street / private equity buying car washes
// and rolling them up into membership chains" is the widely-reported trend; the
// script deliberately quotes no operator margin or price ("varies too much for an
// honest single figure"). No company is named — directional, confirm specifics
// before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  ...
  {dur: 57, kind: 'text', text: 'Private equity|is rolling up.', highlights: ['rolling'], size: 80, media: {src: 'suds/p_pe.jpg', type: 'img', effect: 'in'}},
  {dur: 114, kind: 'text', enter: 'zoom', text: 'Into large|chains.', highlights: ['chains'], size: 92, media: {src: 'suds/p_chains.jpg', type: 'img', effect: 'in'}},
  {dur: 72, kind: 'text', text: 'Two assets,|one purchase.', highlights: ['two'], size: 84, media: {src: 'suds/p_two.jpg', type: 'img', effect: 'in'}},
  {dur: 35, kind: 'text', text: 'The membership|base.', highlights: ['membership'], size: 84, media: {src: 'suds/p_members.jpg', type: 'img', effect: 'in'}},
  {dur: 130, kind: 'text', enter: 'slideL', text: 'And appreciating|real estate.', highlights: ['appreciating'], size: 78, media: {src: 'suds/p_estate.jpg', type: 'img', effect: 'in'}},
...
const SFX: SfxCue[] = [
  {at: 117, src: 'media/sfx_whoosh.mp3', vol: 0.3}, // cut into "no app, no founder" list
  {at: 252, src: 'media/sfx_impact.mp3', vol: 0.5}, // reveal — it's the car wash
  {at: 758, src: 'media/sfx_impact.mp3', vol: 0.45}, // the land holds its value
  {at: 1171, src: 'media/sfx_impact.mp3', vol: 0.5}, // into large chains
  {at: 1560, src: 'media/sfx_impact.mp3', vol: 0.62}, // red card — recurring changes everything
  {at: 1760, src: 'media/sfx_impact.mp3', vol: 0.5}, // close — a subscription with soap
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The car wash roll-up</span>
      </div>
...
// ---------------------------------------------------------------------------
export const SudsReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('suds/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('suds/music.mp3')}
