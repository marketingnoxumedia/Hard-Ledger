...
// ---------------------------------------------------------------------------
// "The storefront was never the business" reel — Amazon's retail storefront
// barely earns; it's the front. Retail carries almost all the revenue and almost
// none of the profit, while the cloud business (AWS) does the earning. The save
// frame lands the point: per segment reporting, AWS is only ~15% of Amazon's
// revenue yet the majority of its operating income, then holds silent before the
// loop closer. Beats are spaced to the client's storyboard (2s / 4s / 4s / 4s /
// 4s / 2s) so each holds its full duration under a continuous voiceover with no
// dead air: the VO is split at its sentence boundaries and each beat placed at
// its mark, with music under the holds. Total 600 frames = ~20s. Mixes three
// video clips (online shopping, order fulfilment, server room) with a store
// still; the save frame and loop closer sit on black. Every background is sourced
...
// 10-K). Rendered as written and NOT independently verified; confirm against the
// latest 10-K before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 60, kind: 'hook', text: 'The storefront|barely earns.', kicker: 'The shop is the front', highlights: ['barely'], size: 84, media: {src: 'amazon/p_15491784.jpg', type: 'img', effect: 'in'}},
  {dur: 120, kind: 'text', text: 'Amazon.|The shop is the front.', highlights: ['front'], size: 80, media: {src: 'amazon/clip_shop.mp4', type: 'video', effect: 'in'}},
  {dur: 120, kind: 'text', enter: 'slideL', text: 'Almost all the revenue.|Almost none|of the profit.', highlights: ['profit'], size: 70, media: {src: 'amazon/clip_warehouse.mp4', type: 'video', effect: 'in'}},
  {dur: 120, kind: 'text', text: 'The cloud|does the earning.', highlights: ['cloud'], size: 84, media: {src: 'amazon/clip_servers.mp4', type: 'video', effect: 'in'}},
  {dur: 120, kind: 'stat', stat: {pre: 'Segment reporting', prefix: '~', value: 15, suffix: '%', post: 'of revenue is AWS —|and most of its operating income', source: 'Amazon 10-K'}},
  {dur: 60, kind: 'text', text: 'The storefront was|never the business.', highlights: ['business'], size: 78},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 60, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 180, src: 'media/sfx_whoosh.mp3', vol: 0.38},
  {at: 300, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 420, src: 'media/sfx_impact.mp3', vol: 0.58},
  {at: 426, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 540, src: 'media/sfx_impact.mp3', vol: 0.55},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The shop is the front</span>
      </div>
...
// ---------------------------------------------------------------------------
export const AmazonReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('amazon/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('amazon/music.mp3')}
