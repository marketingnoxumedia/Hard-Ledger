...
// ---------------------------------------------------------------------------
// "The never-ending payment" reel (WonderReel) — car leasing vs buying, told through the
// numbers. This is an A/B TWIN of NeverstopReel: same message and structure, its OWN fresh
// footage (public/wonder/), and a different hook opener ("Did you ever wonder…" vs "Want to
// know…"). It is the SAME lease-vs-buy topic as LeaseReel, PaymentReel AND NeverstopReel —
// publish only one, or run the twin as a deliberate A/B against NeverstopReel. Two drivers
// with the same $400/month car budget end up thousands apart: over ten years one leases and
// pays every single month, the other buys, finances for five years, then drives five years
// for free. A lease feels smart — a newish car, a fixed cost, nothing up front — so the
// payment never feels like the problem; but lease continuously and it never ends. Ten years
// in: 120 payments and you own nothing. Buy: about 60 payments, then the loan ends and the
// cost drops toward zero. Leasing rents access permanently — you pay for the newest model,
// not for something you keep; the lever was never which car, it's whether the payment ever
// stops. Numbers-driven: three count-up `stat` beats ($400/mo, 120 payments, 60 payments).
// A short ~1.5s pause after the hook (house convention), eight black beats for the
...
// of the two structures, not a quote; specific lease terms, rates and resale move the size
// of the gap, not its direction; verify before publishing. Imagery is generic (no car-brand
...
const SCENES: SceneDef[] = [
  ...
  {dur: 90, kind: 'text', enter: 'slideL', text: 'A lease|feels smart.', highlights: ['smart'], size: 100, media: {src: 'wonder/p_newcar.jpg', type: 'img', effect: 'in'}},
  {dur: 29, kind: 'text', text: 'Nothing huge|up front.', highlights: ['front'], size: 94, media: {src: 'wonder/p_leasedeal.jpg', type: 'img', effect: 'in'}},
  {dur: 132, kind: 'text', text: 'Feels normal —|never the problem.', highlights: ['normal'], size: 88},
  {dur: 81, kind: 'text', text: 'But the payment|never ends.', highlights: ['never'], size: 96},
  {dur: 40, kind: 'stat', stat: {pre: 'TEN YEARS IN', value: 120, post: 'PAYMENTS'}, media: {src: 'wonder/p_120.jpg', type: 'img', effect: 'in'}},
  {dur: 96, kind: 'text', enter: 'slideL', text: 'And you own|nothing.', highlights: ['nothing'], size: 100, media: {src: 'wonder/p_ownnothing.jpg', type: 'img', effect: 'in'}},
  {dur: 64, kind: 'stat', stat: {pre: 'BUY INSTEAD', value: 60, post: 'PAYMENTS'}},
  {dur: 48, kind: 'text', enter: 'slideL', text: 'Then drive five|years for free.', highlights: ['free'], size: 88, media: {src: 'wonder/v_road.mp4', type: 'video', effect: 'in'}},
  {dur: 39, kind: 'text', text: 'Not one lucky|example.', highlights: ['lucky'], size: 92},
  {dur: 102, kind: 'text', text: 'It\'s the shape|of the deal.', highlights: ['shape'], size: 92, media: {src: 'wonder/v_lot.mp4', type: 'video', effect: 'in'}},
  {dur: 71, kind: 'text', text: 'A loan|ends.', highlights: ['ends'], size: 104, media: {src: 'wonder/p_loanend.jpg', type: 'img', effect: 'in'}},
  {dur: 44, kind: 'text', text: 'The cost drops|toward zero.', highlights: ['zero'], size: 92},
  {dur: 88, kind: 'text', enter: 'slideL', text: 'The leaser|keeps paying.', highlights: ['keeps'], size: 92, media: {src: 'wonder/p_payment.jpg', type: 'img', effect: 'in'}},
  {dur: 110, kind: 'impact', text: 'Leasing rents access.|Permanently.', redBg: true},
  {dur: 50, kind: 'text', text: 'Not something|you keep.', highlights: ['keep'], size: 92, media: {src: 'wonder/p_keys.jpg', type: 'img', effect: 'in'}},
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The never-ending payment</span>
      </div>
...
// ---------------------------------------------------------------------------
export const WonderReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('wonder/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('wonder/music.mp3')}
