...
// ---------------------------------------------------------------------------
// "The rental machine" reel — Cintas rents businesses their own uniforms and
// launders them on recurring contracts, turning a garment into an annuity.
// Durations placed on exact spoken-word timestamps (ElevenLabs alignment) at
// +6% pace. Total 1441 frames = ~48s. The red `impact` card sits mid-reel (the
// "one garment / an annuity" thesis); the reel ends on a service-van beat, not
...
const SCENES: SceneDef[] = [
  {dur: 36, kind: 'hook', text: 'There\'s a|company…', kicker: 'The rental machine', highlights: ['company'], size: 104, media: {src: 'cintas/p_18880644.jpg', type: 'img', effect: 'in'}},
  {dur: 68, kind: 'stat', stat: {pre: 'It makes', prefix: '$', value: 10, suffix: 'B', post: 'a year'}, media: {src: 'cintas/p_11733651.jpg', type: 'img', effect: 'in'}},
  {dur: 65, kind: 'text', text: 'By renting businesses|their own uniforms.', highlights: ['renting'], size: 74, media: {src: 'cintas/p_4484152.jpg', type: 'img', effect: 'in'}},
  {dur: 38, kind: 'text', text: 'You never|buy the shirt.', highlights: ['never'], size: 90, media: {src: 'cintas/p_4440574.jpg', type: 'img', effect: 'in'}},
  {dur: 20, kind: 'text', text: 'You rent it.', highlights: ['rent'], size: 110, media: {src: 'cintas/p_965632.jpg', type: 'img', effect: 'in'}},
  {dur: 18, kind: 'text', text: 'Forever.', highlights: ['forever'], size: 140},
  {dur: 74, kind: 'text', enter: 'slideL', text: 'Cintas supplies|and launders them.', highlights: ['Cintas'], size: 80, media: {src: 'cintas/p_8774414.jpg', type: 'img', effect: 'in'}},
  {dur: 152, kind: 'lines', text: 'Uniforms.|Floor mats.|Restroom supplies.|Safety gear.', highlights: ['safety'], reveal: [6, 44, 84, 118], media: {src: 'cintas/p_36122947.jpg', type: 'img', effect: 'panL'}},
  {dur: 122, kind: 'text', text: 'For a million businesses.|All on recurring|rental contracts.', highlights: ['recurring'], size: 72, media: {src: 'cintas/p_7843970.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'impact', text: 'One garment.|An annuity.', redBg: true},
  {dur: 121, kind: 'text', enter: 'slideR', text: 'Collected, cleaned|and returned —|every week.', highlights: ['every'], size: 72, media: {src: 'cintas/p_8774570.jpg', type: 'img', effect: 'panR'}},
  {dur: 40, kind: 'text', text: 'The revenue recurs|by the service.', highlights: ['recurs'], size: 80, media: {src: 'cintas/p_8774638.jpg', type: 'img', effect: 'in'}},
  {dur: 80, kind: 'text', text: 'Not by a|subscription clause.', highlights: ['subscription'], size: 82, media: {src: 'cintas/p_17682883.jpg', type: 'img', effect: 'in'}},
  {dur: 90, kind: 'text', enter: 'zoom', text: 'Switching means|re-outfitting the|whole workforce.', highlights: ['workforce'], size: 72, media: {src: 'cintas/p_31251590.jpg', type: 'img', effect: 'in'}},
  {dur: 52, kind: 'text', text: 'Enough friction to|hold it for years.', highlights: ['friction'], size: 78, media: {src: 'cintas/p_32845683.jpg', type: 'img', effect: 'in'}},
  {dur: 95, kind: 'stat', stat: {pre: 'More than', value: 1, suffix: 'M+', post: 'business customers'}, media: {src: 'cintas/p_28279109.jpg', type: 'img', effect: 'in'}},
  {dur: 83, kind: 'stat', stat: {pre: 'Revenue', prefix: '$', value: 10.3, decimals: 1, suffix: 'B'}, media: {src: 'cintas/p_8774639.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'stat', stat: {pre: 'Record operating margin', value: 22, suffix: '%', bar: 22}},
  {dur: 52, kind: 'stat', stat: {pre: 'Profit', prefix: '$', value: 1.8, decimals: 1, suffix: 'B'}},
  {dur: 99, kind: 'text', enter: 'zoom', text: 'The recurring service —|not the product —|is the business.', highlights: ['service'], size: 70, media: {src: 'cintas/p_6169135.jpg', type: 'img', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 36, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 227, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 245, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 593, src: 'media/sfx_impact.mp3', vol: 0.6},
  {at: 1039, src: 'media/sfx_whoosh.mp3', vol: 0.45},
  {at: 1134, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1217, src: 'media/sfx_impact.mp3', vol: 0.42},
  {at: 1290, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1342, src: 'media/sfx_whoosh.mp3', vol: 0.45},
];
...
const SceneOutro: React.FC = () => {
  ...
  const {fps} = useVideoConfig();
  const parts = [
    {t: 'Not the shirt.', red: false},
    {t: 'The service.', red: false},
    {t: 'It recurs.', red: true},
  ];
  return (
    <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
        ...
        {parts.map((p, i) => {
          ...
          return (
            <span key={i} style={{fontFamily: HEAD, fontSize: p.red ? 150 : 116, textTransform: 'uppercase', letterSpacing: 1, color: C.ink, opacity: s, transform: `translateY(${interpolate(s, [0, 1], [34, 0])}px)`}}>
              {p.red ? (<>It <span style={{color: C.red, textShadow: SH}}>recurs.</span></>) : p.t}
            </span>
          );
        })}
      </div>
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The rental machine</span>
      </div>
...
// ---------------------------------------------------------------------------
export const CintasReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('cintas/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('cintas/music.mp3')}
