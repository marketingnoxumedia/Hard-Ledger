...
// ---------------------------------------------------------------------------
// "The label, not the liquid" reel — bottled water. You buy it by the case and
// think you're paying for water, but the water is the cheapest thing in the
// bottle — almost every cent goes to everything around it. From the tap the same
// volume costs a fraction of a cent per litre; on the shelf, a dollar or more.
// The gap isn't the liquid — it's packaging, branding, logistics and retail
// margin. You're buying convenience and a label; the contents are the smallest
// cost in the chain, the water a rounding error. The product isn't the water,
// it's everything wrapped around it — price tracks the packaging, not the thing
// inside. The voiceover runs continuously with no dead air and scene durations
// sit on its exact spoken-word timestamps (ElevenLabs alignment) at +6% pace —
// total 1077 frames = ~36s. Mixes two video clips (a running tap, a bottling
// line) with stills (a case of capped bottles, a plastic bottle, a poured glass,
// a bottle-to-glass pour); two per-litre stats (<1¢ tap / $1+ bottled), a
// four-item cost list, the "price tracks the packaging" red impact card mid-tail,
// and a closing <1¢-vs-$1 comparison. Every background is sourced fresh for this
// reel (hard rule).
//
// NOTE (illustration): the per-litre figures are round, illustrative
// order-of-magnitude values — tap water at a fraction of a cent per litre vs
// bottled at roughly a dollar or more per litre — not a quoted price for any
// brand or region. Rendered as written; confirm local figures before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 110, kind: 'hook', text: 'You\'re paying|for water?', kicker: 'The label, not the liquid', highlights: ['water'], size: 84, media: {src: 'bottle/p_case.jpg', type: 'img', effect: 'in'}},
  {dur: 87, kind: 'text', text: 'The water\'s the|cheapest part.', highlights: ['cheapest'], size: 80},
  {dur: 88, kind: 'text', text: 'Every cent buys|everything around it.', highlights: ['everything'], size: 74},
  {dur: 43, kind: 'text', text: 'The price? Barely|about water.', highlights: ['price'], size: 78},
  {dur: 60, kind: 'text', enter: 'zoom', text: 'From the|tap.', highlights: ['tap'], size: 96, media: {src: 'bottle/clip_tap.mp4', type: 'video', effect: 'in'}},
  {dur: 73, kind: 'stat', stat: {pre: 'Tap water, per litre', prefix: '<', value: 1, suffix: '¢'}},
  {dur: 38, kind: 'stat', stat: {pre: 'Bottled, per litre', prefix: '$', value: 1, suffix: '+'}},
  {dur: 41, kind: 'text', text: 'The gap isn\'t|the liquid.', highlights: ['liquid'], size: 86},
  {dur: 108, kind: 'lines', text: 'Packaging.|Branding.|Logistics.|Retail margin.', highlights: ['packaging'], reveal: [6, 30, 54, 78]},
  {dur: 68, kind: 'text', enter: 'slideL', text: 'You buy convenience|and a label.', highlights: ['label'], size: 76, media: {src: 'bottle/p_label.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'The contents:|the smallest cost.', highlights: ['smallest'], size: 78},
  {dur: 32, kind: 'text', text: 'The water is a|rounding error.', highlights: ['rounding'], size: 78, media: {src: 'bottle/p_glass.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', text: 'The product isn\'t|the water.', highlights: ['water'], size: 82},
  {dur: 38, kind: 'text', enter: 'slideR', text: 'It\'s everything|wrapped around it.', highlights: ['wrapped'], size: 78, media: {src: 'bottle/clip_bottling.mp4', type: 'video', effect: 'in'}},
  {dur: 46, kind: 'impact', text: 'Price tracks|the packaging.', redBg: true},
  {dur: 37, kind: 'text', text: 'Not the thing|inside.', highlights: ['inside'], size: 84},
  {dur: 20, kind: 'text', text: 'Same|water.', highlights: ['same'], size: 92, media: {src: 'bottle/p_pour.jpg', type: 'img', effect: 'in'}},
  {dur: 77, kind: 'compare', compare: {a: {value: 1, prefix: '<', suffix: '¢', label: 'tap'}, b: {value: 1, prefix: '$', suffix: '/L', label: 'bottled'}, note: 'Same water, per litre'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 328, src: 'media/sfx_whoosh.mp3', vol: 0.35}, // into the tap
  {at: 388, src: 'media/sfx_impact.mp3', vol: 0.5}, // tap <1¢
  {at: 461, src: 'media/sfx_impact.mp3', vol: 0.55}, // bottled $1+
  {at: 466, src: 'media/sfx_chaching.mp3', vol: 0.42},
  {at: 897, src: 'media/sfx_impact.mp3', vol: 0.62}, // price tracks the packaging
  {at: 1000, src: 'media/sfx_impact.mp3', vol: 0.5}, // closing compare
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The label, not the liquid</span>
      </div>
...
// ---------------------------------------------------------------------------
export const BottleReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('bottle/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('bottle/music.mp3')}
