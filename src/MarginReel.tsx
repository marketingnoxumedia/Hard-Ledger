...
// ---------------------------------------------------------------------------
// "Margin tells the business" reel — revenue vs margin. Two companies each ring
// up a dollar of sales; one keeps about two cents, the other about a third. Same
// dollar in, wildly different amounts left behind. Revenue is the size of the
// pipe; margin is what actually stays, and two identical top lines can hide an
// order-of-magnitude gap. A grocer like Kroger keeps roughly one to two cents per
// dollar (competition and perishables leave almost no room); enterprise software
// like Microsoft keeps around thirty-five cents, because serving one more
// customer costs almost nothing. Volume hides the gap — the grocer moves enormous
// revenue to earn what the software firm earns on a fraction of it. Revenue tells
// you the size; margin tells you the business. The voiceover runs continuously
// with no dead air and scene durations sit on its exact spoken-word timestamps
// (ElevenLabs alignment) at +6% pace — total 1316 frames = ~44s. Mixes two video
// clips (a checkout belt, a grocery aisle) with stills (dollar bills, a network
// of pipes, a produce case, a server rack, a stocked warehouse, scattered coins);
// two margin stats (2¢ / 35¢), the "margin tells the business" red impact card
// mid-tail, and a closing 2¢-vs-35¢-kept comparison. Every background is sourced
// fresh for this reel (hard rule).
//
// NOTE (unverified): this reel names real companies (Kroger, NYSE: KR; Microsoft,
// NASDAQ: MSFT) and characterises their net margins — a grocer at ~1–2¢ on the
// dollar, enterprise software at ~35¢ — as illustrative, order-of-magnitude
// figures per the client script. Rendered as written and NOT independently
// verified; confirm against current filings before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 78, kind: 'hook', text: 'Two companies.|One dollar each.', kicker: 'Margin tells the business', highlights: ['dollar'], size: 80, media: {src: 'margin/clip_checkout.mp4', type: 'video', effect: 'in'}},
  {dur: 46, kind: 'text', text: 'One keeps|two cents.', highlights: ['two'], size: 88},
  {dur: 46, kind: 'text', text: 'The other keeps|about a third.', highlights: ['third'], size: 78, media: {src: 'margin/p_cash.jpg', type: 'img', effect: 'in'}},
  {dur: 96, kind: 'text', text: 'Same dollar in.|Different left behind.', highlights: ['different'], size: 74},
  {dur: 34, kind: 'text', text: 'Revenue is|the pipe.', highlights: ['pipe'], size: 92, media: {src: 'margin/p_pipes.jpg', type: 'img', effect: 'in'}},
  {dur: 116, kind: 'text', text: 'Margin is|what stays.', highlights: ['stays'], size: 88},
  {dur: 48, kind: 'text', text: 'An order-of-|magnitude gap.', highlights: ['gap'], size: 80},
  {dur: 72, kind: 'text', enter: 'zoom', text: 'A grocer|like Kroger.', highlights: ['kroger'], size: 84, media: {src: 'margin/clip_aisle.mp4', type: 'video', effect: 'in'}},
  {dur: 53, kind: 'stat', stat: {pre: 'Kroger keeps', value: 2, suffix: '¢', post: 'per dollar · net'}},
  {dur: 121, kind: 'text', enter: 'slideL', text: 'Competition.|Perishables.', highlights: ['perishables'], size: 80, media: {src: 'margin/p_produce.jpg', type: 'img', effect: 'in'}},
  {dur: 37, kind: 'text', text: 'Software|like Microsoft.', highlights: ['microsoft'], size: 80, media: {src: 'margin/p_data.jpg', type: 'img', effect: 'in'}},
  {dur: 92, kind: 'stat', stat: {pre: 'Microsoft keeps', value: 35, suffix: '¢', post: 'per dollar · net'}},
  {dur: 44, kind: 'text', text: 'One more customer|costs nothing.', highlights: ['nothing'], size: 76},
  {dur: 60, kind: 'text', text: 'Volume hides|the gap.', highlights: ['volume'], size: 86},
  {dur: 134, kind: 'text', enter: 'slideR', text: 'Enormous revenue.|Tiny margin.', highlights: ['tiny'], size: 80, media: {src: 'margin/p_warehouse.jpg', type: 'img', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'Revenue tells|the size.', highlights: ['revenue'], size: 84},
  {dur: 74, kind: 'impact', text: 'Margin tells|the business.', redBg: true},
  {dur: 50, kind: 'text', enter: 'zoom', text: 'What sticks —|not what rings up.', highlights: ['sticks'], size: 74, media: {src: 'margin/p_coins.jpg', type: 'img', effect: 'in'}},
  {dur: 68, kind: 'compare', compare: {a: {value: 2, suffix: '¢', label: 'kept'}, b: {value: 35, suffix: '¢', label: 'kept'}, note: 'Same dollar in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 124, src: 'media/sfx_impact.mp3', vol: 0.45}, // a third
  {at: 464, src: 'media/sfx_whoosh.mp3', vol: 0.4}, // cut into grocer
  {at: 589, src: 'media/sfx_impact.mp3', vol: 0.55}, // Kroger 2¢
  {at: 747, src: 'media/sfx_impact.mp3', vol: 0.6}, // Microsoft 35¢
  {at: 753, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1124, src: 'media/sfx_impact.mp3', vol: 0.62}, // margin tells the business
  {at: 1248, src: 'media/sfx_impact.mp3', vol: 0.5}, // closing compare
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Margin tells the business</span>
      </div>
...
// ---------------------------------------------------------------------------
export const MarginReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('margin/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('margin/music.mp3')}
