...
// ---------------------------------------------------------------------------
// "The real ROI" reel (ReturnReel) — the second A/B twin on the college-value
// case, distinct from both MajorReel ("what you study") and TuitionReel ("the
// tuition trap" / cost side). This cut leads on the INVESTMENT framing: a degree
// is a price AND a return, the price tripled (real tuition ~3x its 1980 level)
// while pay didn't, so the gap became trillions in student debt — but the part
// that matters is that the return swings hugely by field ($30K for an $80K job vs
// $120K for a $40K one; same word on the diploma, opposite outcome). Opens on a
// question hook ("The same diploma — smartest money, or the worst?") with a ~1.2s
// post-hook pause held on beat 0 (146 -> 182), lands the red "Trillions / Student
// debt" card mid-reel, and closes on a question ("Which number are you actually
// buying?"). Durations on exact ElevenLabs spoken-word timestamps at +6% pace.
// Total 1192 frames = ~39.7s. **Media-dense:** 14 of 17 beats carry footage, with
// only 2 flat-black beats ("Here's the part that matters.", "Opposite outcome.")
// plus the one red card; the close lands on footage. Own footage under
// public/return/ — no asset shared with MajorReel, TuitionReel, or any other reel
// (skill hard rule). **Illustrative figures — verify before publishing:** "~3x
// real tuition since 1980", "trillions in student debt", and the $30K/$80K vs
// $120K/$40K ROI pairs are directional, widely-cited framing, not a single audited
// dataset. Same college-value case as MajorReel and TuitionReel — publish only one
// college reel to a given audience.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  // beat 0 holds through the ~1.2s post-hook pause spliced after "…or the worst?" (146 -> 182).
  {dur: 182, kind: 'hook', text: 'The same diploma.|Smartest money, or worst?', kicker: 'The real ROI', highlights: ['worst?'], size: 72, media: {src: 'return/p_coinflip.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', text: 'A degree isn\'t|one number.', highlights: ['number'], size: 86, media: {src: 'return/p_pricetag.jpg', type: 'img', effect: 'in'}},
  {dur: 54, kind: 'text', text: 'It\'s a price,|and a return.', highlights: ['return'], size: 84, media: {src: 'return/p_arrows.jpg', type: 'img', effect: 'in'}},
  {dur: 123, kind: 'stat', stat: {pre: 'After inflation, tuition is', value: 3, suffix: 'x', post: 'the 1980 price'}, media: {src: 'return/v_climb.mp4', type: 'video', effect: 'in'}},
  {dur: 48, kind: 'text', text: 'But pay didn\'t|triple with it.', highlights: ['triple'], size: 84, media: {src: 'return/p_wages.jpg', type: 'img', effect: 'in'}},
  {dur: 60, kind: 'text', enter: 'slideL', text: 'A summer job|once funded a year.', highlights: ['once'], size: 80, media: {src: 'return/p_summerjob2.jpg', type: 'img', effect: 'in'}},
  {dur: 44, kind: 'text', text: 'Now it takes|years of debt.', highlights: ['debt'], size: 82, media: {src: 'return/p_debtweight.jpg', type: 'img', effect: 'in'}},
  {dur: 78, kind: 'impact', text: 'Trillions.|In student debt.', redBg: true},
  {dur: 36, kind: 'text', text: 'Here\'s the part|that matters.', highlights: ['matters'], size: 84},
  {dur: 67, kind: 'text', enter: 'slideL', text: 'The return swings|by field.', highlights: ['field'], size: 82, media: {src: 'return/v_diverge.mp4', type: 'video', effect: 'in'}},
  {dur: 86, kind: 'text', text: 'Borrow $30K,|earn $80K.', highlights: ['$80k'], size: 82, media: {src: 'return/p_highroi.jpg', type: 'img', effect: 'in'}},
  {dur: 86, kind: 'text', text: 'Or $120K,|earn $40K.', highlights: ['$120k'], size: 82, media: {src: 'return/p_lowroi.jpg', type: 'img', effect: 'in'}},
  {dur: 43, kind: 'text', text: 'Same word|on the diploma.', highlights: ['same'], size: 84, media: {src: 'return/p_diploma2.jpg', type: 'img', effect: 'in'}},
  {dur: 34, kind: 'text', text: 'Opposite|outcome.', highlights: ['opposite'], size: 90},
  {dur: 44, kind: 'text', text: 'It was never|college or not.', highlights: ['never'], size: 82, media: {src: 'return/p_classroom.jpg', type: 'img', effect: 'in'}},
  {dur: 54, kind: 'text', enter: 'slideL', text: 'This price,|for this return.', highlights: ['price'], size: 84, media: {src: 'return/p_ledger.jpg', type: 'img', effect: 'in'}},
  {dur: 104, kind: 'text', enter: 'zoom', text: 'Which number are you|actually buying?', highlights: ['buying?'], size: 78, media: {src: 'return/v_grad2.mp4', type: 'video', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
// NOTE: a 36-frame (~1.2s) pause is spliced after the hook question (frame 146);
// the whoosh sits on the post-pause reveal, impacts on the 3x reveal, the red
// "trillions" card and the $80K ROI beat, whoosh on the close.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 182, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 285, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 560, src: 'media/sfx_impact.mp3', vol: 0.6},
  {at: 741, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1088, src: 'media/sfx_whoosh.mp3', vol: 0.4},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The real ROI</span>
      </div>
...
// ---------------------------------------------------------------------------
export const ReturnReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('return/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('return/music.mp3')}
