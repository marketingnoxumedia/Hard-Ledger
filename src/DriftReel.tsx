...
// ---------------------------------------------------------------------------
// "Drift is the default" reel — a long-form take on subscription creep. Ask
// people what they spend on subscriptions and they guess ~$86/month; the average
// US household actually runs ~$273/month (~$3,300/year) — streaming, apps,
// memberships, cloud storage — small recurring charges designed to stay below
// the level where you'd notice and cancel. Most name less than half of what they
// pay, ~4 in 10 pay for a subscription they no longer use, and it accrues $9 and
// $14 at a time on cards no one checks. The voiceover runs continuously with no
// dead air and scene durations sit on its exact spoken-word timestamps
// (ElevenLabs alignment) at +6% pace — total 1719 frames = ~57s. Mixes four
// video clips (app store, card tap, coins, small change) with stills; two
// count-up comparison beats ($86 vs $273, $86 vs $3,300), the "drift is the
// default" red impact card, and several beats on black. Every background is
// sourced fresh for this reel (hard rule).
//
// NOTE (unverified): the subscription figures — ~$86 guessed, ~$273/month,
// ~$3,300/year, ~4 in 10 with an unused subscription — echo West Monroe and
// similar surveys and are rendered as written in the client script; they have
// NOT been independently verified — confirm the sources and current figures
// before publishing.
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 74, kind: 'hook', text: 'Ask people what|they spend.', kicker: 'Drift is the default', highlights: ['spend'], size: 78, media: {src: 'drift/p_5900034.jpg', type: 'img', effect: 'in'}},
  {dur: 85, kind: 'text', text: 'They\'ll guess|$86 a month.', highlights: ['$86'], size: 84, media: {src: 'drift/p_89955.jpg', type: 'img', effect: 'in'}},
  {dur: 44, kind: 'text', text: 'Off by|more than half.', highlights: ['half'], size: 88},
  {dur: 104, kind: 'text', text: 'The gap runs|on autopay.', highlights: ['autopay'], size: 82, media: {src: 'drift/clip_card.mp4', type: 'video', effect: 'in'}},
  {dur: 99, kind: 'stat', stat: {pre: 'Actually, per month', prefix: '$', value: 273, post: 'the average US household'}, media: {src: 'drift/p_5466807.jpg', type: 'img', effect: 'in'}},
  {dur: 82, kind: 'lines', text: 'Streaming.|Apps.|Memberships.|Cloud storage.', highlights: ['cloud'], reveal: [4, 22, 40, 58], media: {src: 'drift/clip_apps.mp4', type: 'video', effect: 'in'}},
  {dur: 45, kind: 'text', text: 'Small recurring|charges.', highlights: ['recurring'], size: 84},
  {dur: 94, kind: 'text', enter: 'slideL', text: 'Designed to stay|below notice.', highlights: ['below'], size: 78, media: {src: 'drift/p_7821937.jpg', type: 'img', effect: 'in'}},
  {dur: 128, kind: 'compare', compare: {a: {value: 86, prefix: '$', label: 'guessed'}, b: {value: 273, prefix: '$', label: 'actual'}, note: 'per month · US household', source: 'Source: West Monroe'}},
  {dur: 165, kind: 'text', text: 'Most name|less than half.', highlights: ['less'], size: 80, media: {src: 'drift/p_6919751.jpg', type: 'img', effect: 'in'}},
  {dur: 136, kind: 'stat', stat: {pre: 'A year, on autopay', prefix: '$', value: 3300, comma: true, post: 'much of it forgotten'}, media: {src: 'drift/p_14820411.jpg', type: 'img', effect: 'in'}},
  {dur: 40, kind: 'text', text: 'Things you|forgot.', highlights: ['forgot'], size: 92, media: {src: 'drift/p_5900089.jpg', type: 'img', effect: 'in'}},
  {dur: 143, kind: 'stat', stat: {pre: 'Pay for one unused', value: 4, post: 'in ten admit it'}},
  {dur: 73, kind: 'text', text: 'Small enough|to ignore.', highlights: ['ignore'], size: 84, media: {src: 'drift/clip_coins.mp4', type: 'video', effect: 'in'}},
  {dur: 35, kind: 'text', text: 'Large enough|to matter.', highlights: ['matter'], size: 88},
  {dur: 42, kind: 'impact', text: 'Drift is|the default.', redBg: true},
  {dur: 113, kind: 'text', text: 'Nobody decides to|spend $3,000.', highlights: ['nobody'], size: 78},
  {dur: 67, kind: 'text', text: 'Nine and fourteen,|at a time.', highlights: ['fourteen'], size: 78, media: {src: 'drift/clip_cash.mp4', type: 'video', effect: 'in'}},
  {dur: 48, kind: 'text', text: 'On cards|no one checks.', highlights: ['checks'], size: 84},
  {dur: 102, kind: 'compare', compare: {a: {value: 86, prefix: '$', label: 'guessed'}, b: {value: 3300, prefix: '$', label: 'real'}, note: 'monthly guess · yearly total'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 203, src: 'media/sfx_whoosh.mp3', vol: 0.38},
  {at: 307, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 406, src: 'media/sfx_whoosh.mp3', vol: 0.35},
  {at: 627, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 920, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 926, src: 'media/sfx_chaching.mp3', vol: 0.5},
  {at: 1096, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1347, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1617, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 1623, src: 'media/sfx_chaching.mp3', vol: 0.5},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Drift is the default</span>
      </div>
...
// ---------------------------------------------------------------------------
export const DriftReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('drift/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('drift/music.mp3')}
