...
// ---------------------------------------------------------------------------
// "The grind myth" reel (third take on the hours-worked vs productivity idea, after
// HoursReel and OutputReel — same body, reworded "grinding longest" hook, fresh
// footage). Effort is
// what you can control and busyness reads as productivity, and hours are simple to
// count while value isn't — but across countries longer average hours don't track
// higher output; several nations work fewer hours per worker yet produce more per
// hour. Past a point extra hours bring fatigue and diminishing returns: a rested
// worker out-produces an exhausted one on the same task in less time, because output
// is capped by focus and energy, not the clock. Treating hours as the goal confuses
// effort with results — the lever was never the length of the day, it's the value
// inside each hour. Durations placed on exact spoken-word timestamps (ElevenLabs
// alignment) at +6% pace. Total 1590 frames = ~53.0s. `enter` adds a transition on
...
// stat (it's a concept), two black beats, a red "the lever was never the length of
// the day" card near the tail, and a close on footage (a rhetorical question) rather
// than the red card. Illustrative principle — the cross-country productivity claim
...
const SCENES: SceneDef[] = [
  ...
  {dur: 28, kind: 'text', text: 'Hours are easy|to count.', highlights: ['count'], size: 90, media: {src: 'grind/p_timeclock.jpg', type: 'img', effect: 'in'}},
  {dur: 63, kind: 'text', text: 'Value|isn\'t.', highlights: ['value'], size: 108},
  {dur: 115, kind: 'text', text: 'Longer hours|don\'t track output.', highlights: ['longer'], size: 88, media: {src: 'grind/p_flatline.jpg', type: 'img', effect: 'in'}},
  {dur: 108, kind: 'text', enter: 'slideL', text: 'Fewer hours.|More per hour.', highlights: ['fewer'], size: 90, media: {src: 'grind/p_calm.jpg', type: 'img', effect: 'in'}},
  {dur: 25, kind: 'text', text: 'Than places|that grind.', highlights: ['grind'], size: 96},
  {dur: 72, kind: 'text', text: 'Past a point,|fatigue hits.', highlights: ['fatigue'], size: 90, media: {src: 'grind/p_tired.jpg', type: 'img', effect: 'in'}},
  {dur: 40, kind: 'text', text: 'Diminishing|returns.', highlights: ['diminishing'], size: 92, media: {src: 'grind/p_curve.jpg', type: 'img', effect: 'in'}},
  {dur: 110, kind: 'text', text: 'A rested worker|out-produces.', highlights: ['rested'], size: 90, media: {src: 'grind/p_rested.jpg', type: 'img', effect: 'in'}},
  {dur: 58, kind: 'text', text: 'Same task.|Less time.', highlights: ['less'], size: 100},
  {dur: 33, kind: 'text', text: 'Capped by focus|and energy.', highlights: ['energy'], size: 88, media: {src: 'grind/p_focus.jpg', type: 'img', effect: 'in'}},
  {dur: 67, kind: 'text', text: 'Not by|the clock.', highlights: ['clock'], size: 100, media: {src: 'grind/p_clock2.jpg', type: 'img', effect: 'in'}},
  {dur: 72, kind: 'text', enter: 'slideL', text: 'Hours as the goal|confuses it.', highlights: ['confuses'], size: 88, media: {src: 'grind/p_maze.jpg', type: 'img', effect: 'in'}},
  {dur: 59, kind: 'text', text: 'It was never|how long.', highlights: ['long'], size: 92, media: {src: 'grind/p_hourglass.jpg', type: 'img', effect: 'in'}},
...
const SFX: SfxCue[] = [
  {at: 101, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 412, src: 'media/sfx_impact.mp3', vol: 0.45},
  {at: 590, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 835, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1285, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1512, src: 'media/sfx_impact.mp3', vol: 0.5},
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>The grind myth</span>
      </div>
...
export const GrindReel: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('grind/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('grind/music.mp3')}
