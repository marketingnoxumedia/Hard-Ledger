...
// ---------------------------------------------------------------------------
// "Owners, not earners" reel (RallyReel) — US stock-ownership / wealth concentration. You
// think the stock market is everyone's market; it isn't. The top 10% of households own about
// 90% of all stocks, the bottom half roughly 1%. The line that divides wealth isn't mainly
// income — it's who owns the assets that compound. When the market rises 20%, almost all of
// that gain flows to the top 10% who already hold nearly every share, so a booming market and
// a struggling household sit in the same year. At the very top, the wealthiest 1% now hold a
// record ~31% of all household wealth: wages get taxed and spent, assets grow on themselves,
// and ownership pulls further ahead every year. Markets reward owners, not earners. Numbers-
// driven: four count-up `stat` beats (90% / 1% / 20% / 31%). A short ~1.5s pause after the
// hook (house convention), eight black beats for the punch-lines, one red "markets reward
// owners, not earners" `impact` card as the mid-tail thesis, and a rhetorical question close
// on footage (a financial-district skyline) rather than the red card. Durations on exact
// spoken-word timestamps (ElevenLabs alignment) at +6% pace. Total 1614 frames = ~53.8s.
// `enter` adds a transition on some cuts. Every scene uses a distinct background sourced fresh
// for THIS reel (public/rally/) — no footage shared with any other reel (hard rule). NOTE:
// this is the SAME topic as the existing OwnershipReel ("the ownership gap") — publish only
// one, or run this as a deliberate A/B / refreshed take. FLAG: real figures — the ~90% of
// stocks / ~1% for the bottom half / record ~31% for the top 1% are widely-cited US wealth-
// concentration numbers but are round approximations that drift with the data vintage; the
// "market rises 20%" is illustrative. Verify before publishing. Imagery is generic (no legible
...
const SCENES: SceneDef[] = [
  ...
  {dur: 59, kind: 'text', enter: 'slideL', text: 'It\'s who owns the assets|that compound.', highlights: ['compound'], size: 82, media: {src: 'rally/p_compound.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'stat', stat: {pre: 'THE MARKET RISES', value: 20, suffix: '%'}, media: {src: 'rally/v_rally.mp4', type: 'video', effect: 'in'}},
  {dur: 100, kind: 'text', text: 'Almost all of it|flows to the top.', highlights: ['top'], size: 84, media: {src: 'rally/p_luxury.jpg', type: 'img', effect: 'in'}},
  {dur: 55, kind: 'text', text: 'Already holding|nearly every share.', highlights: ['every'], size: 84, media: {src: 'rally/p_share.jpg', type: 'img', effect: 'in'}},
  {dur: 77, kind: 'text', enter: 'slideL', text: 'A boom, and a|struggling household.', highlights: ['struggling'], size: 82, media: {src: 'rally/p_worker.jpg', type: 'img', effect: 'in'}},
  {dur: 25, kind: 'text', text: 'Same country,|same year.', highlights: ['year'], size: 92},
  {dur: 120, kind: 'text', text: 'And it goes further|at the very top.', highlights: ['further'], size: 84},
  {dur: 92, kind: 'stat', stat: {pre: 'THE TOP 1% HOLD', value: 31, suffix: '%', post: 'OF ALL WEALTH'}, media: {src: 'rally/p_wealth.jpg', type: 'img', effect: 'in'}},
  {dur: 45, kind: 'text', text: 'Wages get taxed|and spent.', highlights: ['spent'], size: 88, media: {src: 'rally/p_bills.jpg', type: 'img', effect: 'in'}},
  {dur: 50, kind: 'text', text: 'Assets grow|on themselves.', highlights: ['grow'], size: 90, media: {src: 'rally/p_growth.jpg', type: 'img', effect: 'in'}},
  {dur: 93, kind: 'text', enter: 'slideL', text: 'Ownership pulls|further ahead.', highlights: ['ahead'], size: 86},
  {dur: 68, kind: 'text', text: 'Not out-earning|everyone.', highlights: ['outearning'], size: 90},
  {dur: 51, kind: 'text', text: 'Things that grow|while they sleep.', highlights: ['sleep'], size: 84, media: {src: 'rally/v_sleep.mp4', type: 'video', effect: 'in'}},
  {dur: 81, kind: 'impact', text: 'Markets reward owners.|Not earners.', redBg: true},
  {dur: 102, kind: 'text', enter: 'slideL', text: 'Watching from the|wrong side.', highlights: ['wrong'], size: 86, media: {src: 'rally/p_line.jpg', type: 'img', effect: 'in'}},
  {dur: 47, kind: 'text', text: 'Most households|feel nothing.', highlights: ['nothing'], size: 88},
  {dur: 33, kind: 'text', enter: 'zoom', text: 'Who was it|really for?', highlights: ['for?'], size: 90, media: {src: 'rally/v_close.mp4', type: 'video', effect: 'in'}},
];
...
const SFX: SfxCue[] = [
  ...
  {at: 952, src: 'media/sfx_impact.mp3', vol: 0.55},  // 31% stat
  {at: 1351, src: 'media/sfx_impact.mp3', vol: 0.72}, // red thesis card
  {at: 1581, src: 'media/sfx_whoosh.mp3', vol: 0.4},  // close
];
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Owners, not earners</span>
      </div>
...
// ---------------------------------------------------------------------------
export const RallyReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('rally/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('rally/music.mp3')}
