...
// ---------------------------------------------------------------------------
// "Pay yourself first" reel — record household debt and a rock-bottom saving
// rate are one story: when spending outpaces income the gap fills with
// ~21%-rate credit. Wealth-builders remove their slice before they spend.
// Durations placed on exact spoken-word timestamps (ElevenLabs alignment) at
// +6% pace. Total 1256 frames = ~41.9s. The red `impact` card ("wealth isn't
// what's left over") sits at the penultimate beat; the reel ends on a
// coin-into-savings media beat, not the red card. Every scene uses a distinct
// background sourced fresh for this reel — no footage shared with any other
// reel (see the skill's hard rule).
// ---------------------------------------------------------------------------
const SCENES: SceneDef[] = [
  {dur: 98, kind: 'hook', text: 'Americans owe more|than ever before.', kicker: 'Pay yourself first', highlights: ['ever'], size: 78, media: {src: 'payfirst/p_32641818.jpg', type: 'img', effect: 'in'}},
  {dur: 65, kind: 'stat', stat: {pre: 'A record', prefix: '$', value: 18.8, decimals: 1, suffix: 'T'}, media: {src: 'payfirst/p_6266627.jpg', type: 'img', effect: 'in'}},
  {dur: 97, kind: 'text', text: 'While saving less|of each paycheck.', highlights: ['less'], size: 78, media: {src: 'payfirst/p_8719576.jpg', type: 'img', effect: 'in'}},
  {dur: 41, kind: 'text', text: 'Those aren\'t|two problems.', highlights: ['two'], size: 88, media: {src: 'payfirst/p_7534380.jpg', type: 'img', effect: 'in'}},
  {dur: 22, kind: 'text', text: 'They\'re one.', highlights: ['one'], size: 120},
  {dur: 124, kind: 'text', enter: 'slideL', text: 'The saving rate sits|in the low|single digits.', highlights: ['low'], size: 74, media: {src: 'payfirst/p_7054384.jpg', type: 'img', effect: 'panL'}},
  {dur: 100, kind: 'stat', stat: {pre: 'Saving rate, decades ago', value: 10, suffix: '%'}},
  {dur: 54, kind: 'text', text: 'The same story|from two sides.', highlights: ['same'], size: 82, media: {src: 'payfirst/p_9821387.jpg', type: 'img', effect: 'in'}},
  {dur: 73, kind: 'text', enter: 'slideR', text: 'When spending|outpaces income…', highlights: ['spending'], size: 78, media: {src: 'payfirst/p_7318916.jpg', type: 'img', effect: 'in'}},
  {dur: 30, kind: 'text', text: 'The gap fills|with credit.', highlights: ['credit'], size: 84, media: {src: 'payfirst/p_5849580.jpg', type: 'img', effect: 'in'}},
  {dur: 31, kind: 'text', text: 'Month after|month.', highlights: ['month'], size: 92, media: {src: 'payfirst/p_29509491.jpg', type: 'img', effect: 'in'}},
  {dur: 49, kind: 'text', text: 'The cushion that|was savings…', highlights: ['cushion'], size: 78, media: {src: 'payfirst/p_1602726.jpg', type: 'img', effect: 'in'}},
  {dur: 48, kind: 'text', text: '…is now|a balance.', highlights: ['balance'], size: 92, media: {src: 'payfirst/p_210742.jpg', type: 'img', effect: 'in'}},
  {dur: 53, kind: 'stat', stat: {pre: 'Card rates near', value: 21, suffix: '%', bar: 21}, media: {src: 'payfirst/p_11006806.jpg', type: 'img', effect: 'in'}},
  {dur: 109, kind: 'text', enter: 'zoom', text: 'Filling that gap|is expensive.', highlights: ['expensive'], size: 80, media: {src: 'payfirst/p_36633901.jpg', type: 'img', effect: 'in'}},
  {dur: 62, kind: 'text', text: 'Wealth-builders take|their slice first.', highlights: ['first'], size: 76, media: {src: 'payfirst/p_9755383.jpg', type: 'img', effect: 'in'}},
  {dur: 31, kind: 'text', text: 'Then spend|what\'s left.', highlights: ['left'], size: 90, media: {src: 'payfirst/p_7679727.jpg', type: 'img', effect: 'in'}},
  {dur: 39, kind: 'text', text: 'Not the other|way around.', highlights: ['not'], size: 84},
  {dur: 71, kind: 'impact', text: 'Wealth isn\'t|what\'s left over.', redBg: true},
  {dur: 59, kind: 'text', enter: 'zoom', text: 'It\'s removed|before you start.', highlights: ['before'], size: 82, media: {src: 'payfirst/p_12956000.jpg', type: 'img', effect: 'in'}},
];

// Sound-effect cues (frame, file, gain). Placed on key beats, not every cut.
type SfxCue = {at: number; src: string; vol: number};
const SFX: SfxCue[] = [
  {at: 98, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 301, src: 'media/sfx_impact.mp3', vol: 0.55},
  {at: 447, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 601, src: 'media/sfx_whoosh.mp3', vol: 0.4},
  {at: 832, src: 'media/sfx_impact.mp3', vol: 0.5},
  {at: 1126, src: 'media/sfx_impact.mp3', vol: 0.72},
  {at: 1197, src: 'media/sfx_chaching.mp3', vol: 0.5},
];
...
const SceneOutro: React.FC = () => {
  ...
  const {fps} = useVideoConfig();
  const parts = [
    {t: 'Take your slice.', red: false},
    {t: 'Spend the rest.', red: false},
    {t: 'First.', red: true},
  ];
  return (
    <AbsoluteFill style={{justifyContent: 'center', alignItems: 'center'}}>
        ...
        {parts.map((p, i) => {
          ...
          return (
            <span key={i} style={{fontFamily: HEAD, fontSize: p.red ? 150 : 116, textTransform: 'uppercase', letterSpacing: 1, color: C.ink, opacity: s, transform: `translateY(${interpolate(s, [0, 1], [34, 0])}px)`}}>
              {p.red ? (<span style={{color: C.red, textShadow: SH}}>First.</span>) : p.t}
            </span>
          );
        })}
      </div>
  ...
  return (
    <>
      <div style={{position: 'absolute', top: 70, left: 80, display: 'flex', alignItems: 'center', gap: 14}}>
        ...
        <span style={{fontFamily: BODY, fontWeight: 700, fontSize: 22, letterSpacing: 5, color: C.ink, textTransform: 'uppercase', textShadow: SH}}>Pay yourself first</span>
      </div>
...
// ---------------------------------------------------------------------------
export const PayFirstReel: React.FC = () => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{backgroundColor: C.bg}}>
      {HAS_VOICEOVER ? <Audio src={staticFile('payfirst/voiceover.mp3')} /> : null}
      <Audio
        src={staticFile('payfirst/music.mp3')}
