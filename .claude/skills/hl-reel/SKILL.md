...
- **No-repeat media (HARD RULE)**: every footage scene uses a distinct asset,
  ...
  folder — `grep -oE "src: '(media|yahoo|carwash|att|ownership|laundromat|inflation|sci|quibi|waste|lifetime|pest)/[^']+'"
...
- Examples in this repo alongside the original `StorageUnit`: `src/YahooReel.tsx`
  ...
  ownership gap"), `src/LaundromatReel.tsx` ("the laundromat play"), `src/InflationReel.tsx` ("the inflation tax"), `src/SciReel.tsx` ("the invisible roll-up"), `src/QuibiReel.tsx` ("money isn’t demand"), `src/WasteReel.tsx` ("own the landfill"), `src/LifetimeReel.tsx` ("when it starts working"), and
  `src/PestReel.tsx` ("it kills bugs"). Each has
