...
- **No-repeat media (HARD RULE)**: every footage scene uses a distinct asset,
  ...
  folder — `grep -oE "src: '(media|yahoo|carwash|att|ownership|laundromat|inflation|sci|quibi)/[^']+'"
...
- Examples in this repo alongside the original `StorageUnit`: `src/YahooReel.tsx`
  ...
  ownership gap"), `src/LaundromatReel.tsx` ("the laundromat play"), `src/InflationReel.tsx` ("the inflation tax"), `src/SciReel.tsx` ("the invisible roll-up"), and
  `src/QuibiReel.tsx` ("money isn’t demand"). Each has
